//! fractal-cursor-worker — host-side Cursor compatibility worker CLI.
//!
...
fn main() -> ExitCode {
    ...
    let manifest = match WorkerManifest::load(cursor_manifest_path()) {
        Ok(m) => m,
        Err(e) => {
    ...
    match cmd.as_str() {
        "discover-version" => match CursorLauncher::discover_version_live(&manifest) {
            Ok(v) => {
        ...
        "open-session" => {
            ...
            match CursorLauncher::open_session(&manifest, &ws, &prompt, opts) {
                Ok(result) => {
        ...
        "replay-session" => {
            ...
            match CursorLauncher::open_session(&manifest, &ws, "replay", opts) {
                Ok(result) => {
                    println!("{}", serde_json::to_string(&result).unwrap());
                    ExitCode::SUCCESS
                }
        ...
        "cancel-demo" => {
            ...
            let _ = CursorLauncher::request_cancel(&flag, &req);
