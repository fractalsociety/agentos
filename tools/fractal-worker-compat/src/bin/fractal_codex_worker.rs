//! fractal-codex-worker — host-side Codex compatibility worker CLI.
//!
//! Speaks fractal-worker/v1. Credentials arrive only as opaque handle IDs via
//! FRACTAL_CODEX_SECRET_HANDLE (never as embedded secrets).

use fractal_worker_compat::{
    codex_manifest_path, CancelRequest, CodexLauncher, OpenSessionOpts, SecretHandle,
...
fn main() -> ExitCode {
    ...
    let manifest = match WorkerManifest::load(codex_manifest_path()) {
        Ok(m) => m,
        Err(e) => {
    ...
    match cmd.as_str() {
        "discover-version" => match CodexLauncher::discover_version_live(&manifest) {
            Ok(v) => {
        ...
        "open-session" => {
            ...
            match CodexLauncher::open_session(&manifest, &ws, &prompt, opts) {
                Ok(result) => {
        ...
        "replay-session" => {
            ...
            match CodexLauncher::open_session(&manifest, &ws, "replay", opts) {
                Ok(result) => {
                    println!("{}", serde_json::to_string(&result).unwrap());
                    ExitCode::SUCCESS
                }
        ...
        "cancel-demo" => {
            ...
            let _ = CodexLauncher::request_cancel(&flag, &req);
