//! fractal-hermes-worker — host-side Hermes compatibility worker CLI.
//!
//! Speaks fractal-worker/v1. Credentials arrive only as opaque handle IDs via
//! FRACTAL_HERMES_SECRET_HANDLE (never as embedded secrets).

use fractal_worker_compat::{
    hermes_manifest_path, CancelRequest, HermesLauncher, OpenSessionOpts, SecretHandle,
...
fn main() -> ExitCode {
    ...
    let manifest = match WorkerManifest::load(hermes_manifest_path()) {
        Ok(m) => m,
        Err(e) => {
    ...
    match cmd.as_str() {
        "discover-version" => match HermesLauncher::discover_version_live(&manifest) {
            Ok(v) => {
        ...
        "open-session" => {
            ...
            match HermesLauncher::open_session(&manifest, &ws, &prompt, opts) {
                Ok(result) => {
        ...
        "replay-session" => {
            ...
            match HermesLauncher::open_session(&manifest, &ws, "replay", opts) {
                Ok(result) => {
                    println!("{}", serde_json::to_string(&result).unwrap());
                    ExitCode::SUCCESS
                }
        ...
        "cancel-demo" => {
            ...
            let _ = HermesLauncher::request_cancel(&flag, &req);
