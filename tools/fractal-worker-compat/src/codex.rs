//! Codex CLI compatibility adapter for `fractal-worker/v1`.
//!
//! Loads `manifests/workers/codex.toml`, performs real CLI version preflight
//! (`codex --version`, semver expected — stubs rejected), launches the Codex
//! CLI only inside a temporary isolated Git workspace, normalizes native
//! `codex-exec-json` / JSONL output into shared Fractal session events and a common
...
/// Live Codex launcher behind the Fractal worker boundary.
pub struct CodexLauncher;

impl CodexLauncher {
    /// Resolve the Codex executable via path-lookup (never a path embedded in the manifest).
    pub fn resolve_executable(manifest: &WorkerManifest) -> Result<PathBuf> {
        ...
        for name in candidates {
            ...
            if let Ok(path) = which_env_override("FRACTAL_CODEX_EXECUTABLE", &name) {
                return Ok(path);
            }
        }
        Err(err(format!(
            "codex CLI not found on PATH (tried {} and aliases)",
    ...
    pub fn discover_version_live(manifest: &WorkerManifest) -> Result<VersionRecord> {
        ...
        if !output.status.success() {
            ...
            return Err(err(format!(
                "codex version preflight failed (status {:?}): {}",
                output.status.code(),
        ...
        if lower.contains("stub") || lower.contains("fake") {
            return Err(err("codex version preflight returned a stub identity"));
        }
        ...
        {
            return Err(err(format!(
                "codex version preflight got non-semver output: {}",
    ...
    pub fn open_session(
        ...
        let isolated =
            materialize_isolated_workspace("fractal-codex", workspace, opts.seed_dir.as_deref())?;
        let session_id = format!("codex-{}", workspace.workspace_id);
        ...
        let mut events = normalize_codex_stream(&isolated, &raw_stream, child_exit)?;
        if cancelled {
...
fn secret_handle_from_env() -> Option<SecretHandle> {
    let id = std::env::var("FRACTAL_CODEX_SECRET_HANDLE").ok()?;
    if id.is_empty() {
        return None;
    }
    Some(SecretHandle {
...
mod local_tests {
    use super::*;
    use crate::codex_manifest_path;

    fn manifest() -> WorkerManifest {
        WorkerManifest::load(codex_manifest_path()).unwrap()
    }

    fn ws() -> WorkspaceInput {
        WorkspaceInput {
            workspace_id: "codex-unit".into(),
    ...
    #[test]
    fn replay_maps_native_codex_jsonl_to_fractal_result() {
        let result = CodexLauncher::open_session(
            &manifest(),
            &ws(),
            "fix health",
            OpenSessionOpts {
                replay_jsonl: Some(CODEX_JSONL.into()),
                replay_peak_rss_bytes: Some(3 * 1024 * 1024),
                secret: Some(SecretHandle {
                    id: "handle:codex-test".into(),
                    purpose: "cli-auth".into(),
                }),
                ..Default::default()
            },
    ...
    fn replay_denies_file_edits_outside_allowlist() {
        let result = CodexLauncher::open_session(
            ...
            "noop",
            OpenSessionOpts {
                replay_jsonl: Some(CODEX_JSONL.replace("src/health.c", "README.md")),
                ..Default::default()
            },
        )
        .unwrap();
