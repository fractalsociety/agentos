...
const SERVER_NAME: &str = "agentos-control";
const SERVER_VERSION: &str = env!("CARGO_PKG_VERSION");
const DEFAULT_PROTOCOL_VERSION: &str = "2025-06-18";

#[derive(Parser, Debug)]
#[command(name = "agentos-mcp", version, about)]
struct Args {
...
fn tools_list() -> Value {
    json!({"tools": [
        tool_descriptor(
            "agentos_pool_status",
            "Get AgentOS agent pool status",
            "Read the live worker-pool capacity from a booted AgentOS CC-PD.",
            json!({"type": "object", "properties": {}, "additionalProperties": false})
        ),
        tool_descriptor(
            "agentos_list_guests",
            "List AgentOS guests",
            "List guest VMs known to the live AgentOS control plane.",
            json!({"type": "object", "properties": {}, "additionalProperties": false})
        ),
        tool_descriptor(
            "agentos_guest_status",
            "Get AgentOS guest status",
...
fn pool_status(raw: Value) -> std::result::Result<Value, String> {
    ...
    if status != 0 {
        return Err(format!("AgentOS returned pool status error {status}"));
    }
    let accounted = busy
        .checked_add(idle)
    ...
    F: Fn(&str, &[String]) -> std::result::Result<Value, String>,
{
    ...
    let result = match name {
        "agentos_pool_status" => invoke("list-polecats", &[]).and_then(pool_status),
        "agentos_list_guests" => invoke("list-guests", &[]),
        "agentos_guest_status" => {
        ...
        "initialize" => {
            ...
            rpc_result(
                ...
                json!({
                    ...
                    "instructions": "Use these read-only tools for live AgentOS state. Never infer capacity or guest state from source files when a live tool result is available."
                }),
            )
        }
    ...
    fn pool_tool_names_registers_and_derives_faulted_count() {
        let response = handle_request(
            &request(
                3,
                "tools/call",
                json!({"name": "agentos_pool_status", "arguments": {}}),
            ),
            &|command, args| {
    ...
    fn guest_handle_is_validated_before_agentctl() {
        let response = handle_request(
            &request(
                5,
                "tools/call",
                json!({"name": "agentos_guest_status", "arguments": {"guest_handle": -1}}),
            ),
            &|_, _| unreachable!(),
        )
        .unwrap();
