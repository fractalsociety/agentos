            ...
            Some(pv) => {
                let valid = match pv {
                    serde_yaml::Value::Number(n) => {
                        n.as_i64().map(|i| (0..=254).contains(&i)).unwrap_or(false)
                    }
                    _ => false,
                };
                if !valid {
            ...
            match pnm {
                ...
                Some(p) => {
                    if !pd_names.contains(p) {
                        errors.push(format!("Channel '{}' references unknown PD '{}'", cname, p));
                    }
                }
            }
...
fn emit_sdf(topo: &Topology) -> String {
    ...
    lines.push("  Generated by gen_sdf.py - FractalOS Microkit SDF".to_string());
    lines.push(format!(
        "  PDs: {}, Channels: {}, MRs: {}",
        pds.len(),
        chans.len(),
    ...
    sorted_chans.sort_by(|a, b| {
        let id_a_a = a.id_a.as_ref().and_then(channel_id_as_int).unwrap_or(999);
        let id_a_b = b.id_a.as_ref().and_then(channel_id_as_int).unwrap_or(999);
...
#[derive(Parser)]
#[command(name = "gen-sdf", about = "FractalOS Microkit SDF generator")]
struct Cli {
    /// YAML topology spec
    topology: PathBuf,
...
fn main() -> Result<()> {
    ...
    let mut topo: Topology =
        serde_yaml::from_str(&content).with_context(|| "error: YAML parse error")?;
...
mod tests {
    ...
    fn make_channel(name: &str, pd_a: &str, id_a: u32, pd_b: &str, id_b: u32) -> Channel {
        Channel {
    ...
    fn test_validate_empty() {
        ...
        assert!(
            errors.is_empty(),
            "empty topology should be valid, got: {:?}",
            errors
        );
    }
