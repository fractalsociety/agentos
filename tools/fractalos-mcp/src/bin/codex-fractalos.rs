//! Launch the official Codex CLI with the read-only FractalOS MCP bridge.
//!
...
#[derive(Parser, Debug)]
#[command(name = "codex-fractalos", version, trailing_var_arg = true)]
struct Args {
    /// Official Codex CLI executable.
    #[arg(long, default_value = "codex")]
    codex: PathBuf,

    /// FractalOS MCP server executable. Defaults to the launcher's sibling.
    #[arg(long)]
    mcp: Option<PathBuf>,
...
fn default_mcp_path() -> Result<PathBuf> {
    let launcher = std::env::current_exe().context("cannot resolve codex-fractalos executable")?;
    ...
    Ok(launcher
        .parent()
        .context("codex-fractalos executable has no parent directory")?
        .join(name))
}
...
fn main() {
    match run() {
        ...
        Err(error) => {
            eprintln!("codex-fractalos: {error:#}");
            std::process::exit(1);
        }
    }
}
