...
This contract defines the low-level IPC wire format used by every FractalOS
...
the time the root task (or capability broker) grants it. FractalOS packs three
...
(0) means success. Values 1–7 are generic FractalOS errors. Service-specific
error codes begin at `0x0100`.

## Compatibility

All FractalOS service contracts include this header and build on `sel4_msg_t`.
