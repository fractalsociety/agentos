...
FractalOS.  Agents generate or retrieve new WASM implementations and submit
...
fractalos.capabilities section, capability declarations
    │
    ▼  VSWAP_OP_COMMIT
...
(Note: opcodes also defined as `OP_VIBE_*` in `fractalos.h` for kernel dispatch)
...
- `kernel/fractalos-root-task/src/vibe_engine.c` — PD implementation
- `kernel/fractalos-root-task/include/fractalos.h` — OP_VIBE_* constants
