# agentOS mesh-controller guest role
...
under `/var/db/agentos-secrets/headscale` with mode 0700 and are never placed in
the guest-role image. Headscale policy and `netcap-map.json` are deny-by-default
and map authenticated tailnet tags to agentOS NetCap classes.
...
The default endpoint is `https://mesh.agentos.internal:8080`; publish that DNS
