...
#[test]
fn codex_manifest_loads_without_shell_network_filesystem_authority_or_secrets() {
    assert_worker_wit_contract();
    let text = fs::read_to_string(manifest_path("codex")).expect("read manifest");
    scan_manifest_safety(&text).expect("manifest safety scan");
    assert!(text.contains("native_output_format = \"codex-exec-json\""));
    let m = load_manifest("codex");
    assert_eq!(m.worker.id, "codex");
    assert_eq!(m.worker.provider, "codex");
    assert_eq!(m.worker.protocol, "fractal-worker/v1");
    assert_eq!(m.worker.executable.name, "codex");
...
#[test]
fn codex_manifest_requires_real_version_preflight_and_accepts_non_stub_cli_identity() {
    let manifest = load_manifest("codex");
    let version = preflight_version_record("codex", &manifest, "codex 0.1.0");
    assert_eq!(version.cli_version, "0.1.0");
}

#[test]
fn codex_version_output_parser_records_real_cli_identity() {
    let version = fixture_version("codex", "codex 0.1.0");
    assert_eq!(version.worker_id, "codex");
    assert_eq!(version.cli_name, "codex");
    assert_eq!(version.cli_version, "0.1.0");
}

#[test]
fn codex_isolated_repository_input_rejects_absolute_and_parent_paths() {
...
#[test]
fn codex_jsonl_normalizes_to_shared_fractal_events() {
    let events = normalize_jsonl_events(canonical_jsonl()).expect("normalize Codex native JSONL");
...
#[test]
fn codex_cancellation_maps_to_terminal_cancelled_status() {
    let mut cancelled =
...
#[test]
fn codex_terminal_status_peak_rss_and_allowed_files_enter_result() {
    let result = terminal_result_for("codex", "codex 0.1.0");
...
#[test]
fn codex_allowed_file_enforcement_rejects_out_of_scope_edits() {
...
#[test]
fn codex_opaque_secret_handling_redacts_values_but_preserves_handle() {
    let manifest = load_manifest("codex");
    ...
    let result = terminal_result_for("codex", "0.1.0");
    assert_eq!(
        result.secret_handle.as_ref().map(|h| h.id.as_str()),
        Some("handle:codex-opaque-test")
    );
}

#[test]
fn codex_result_semantics_match_claude_despite_different_cli_output_format() {
    let claude = terminal_result_for("claude", "Claude Code 1.2.3");
    let codex = terminal_result_for("codex", "codex 0.1.0");
    assert_eq!(semantic_projection(&codex), semantic_projection(&claude));
}
