/*
 * freebsd_vmm_test.c — contract tests for the FreeBSDVMM PD
 *
 * Covered opcodes:
 *   MSG_VM_CREATE                  — create a FreeBSD VM
...
void run_freebsd_vmm_tests(microkit_channel ch)
{
    ...
    (void)microkit_ppcall(ch, microkit_msginfo_new(MSG_VM_STATUS, 2));
    {
        ...
        if (rc == AOS_OK || rc == AOS_ERR_NOT_FOUND || rc == AOS_ERR_INVAL) {
            _tf_ok("freebsd_vmm: STATUS with bogus id returns ok/not-found/inval");
        } else {
            _tf_fail_point("freebsd_vmm: STATUS with bogus id returns ok/not-found/inval",
    ...
    (void)microkit_ppcall(ch, microkit_msginfo_new(MSG_VM_DESTROY, 2));
    {
        ...
        if (rc == AOS_ERR_NOT_FOUND || rc == AOS_ERR_INVAL) {
            _tf_ok("freebsd_vmm: DESTROY with bogus id returns not-found/inval");
        } else {
            _tf_fail_point("freebsd_vmm: DESTROY with bogus id returns not-found/inval",
                           "unexpected error code");
        }
    }

    /* CREATE — no loader staged; expect UNIMPL or INVAL */
    ASSERT_IPC_OK_OR_ERR(ch, MSG_VM_CREATE, AOS_ERR_UNIMPL,
                         "freebsd_vmm: CREATE returns ok or unimpl");
}
