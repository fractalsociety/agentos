#!/bin/sh
# Genuine official-Codex coding loop against a live FractalOS CC-PD.
...
LAUNCHER=${CODEX_RUN_AGENT:-${CODEX_FRACTALOS:-"$ROOT/target/release/run-agent"}}
...
[ "${FRACTALOS_CODEX_LIVE:-0}" = "1" ] || skip "set FRACTALOS_CODEX_LIVE=1 to spend model tokens"
...
[ -S "$SOCKET" ] || skip "FractalOS CC-PD socket is not live: $SOCKET"
...
WORK=$(mktemp -d "${TMPDIR:-/tmp}/fractalos-codex-e2e.XXXXXX")
ARTIFACTS=$(mktemp -d "${TMPDIR:-/tmp}/fractalos-codex-result.XXXXXX")
trap 'rm -rf "$WORK" "$ARTIFACTS"' EXIT HUP INT TERM
cp "$FIXTURE"/* "$WORK"/
git -C "$WORK" init -q
git -C "$WORK" config user.name "FractalOS E2E"
git -C "$WORK" config user.email "fractalos-e2e@invalid"
git -C "$WORK" add .
...
"$LAUNCHER" \
    ...
    --metrics \
    --prompt \
    'Call fractalos_pool_status exactly once and report its named live counts. Fix only agent_health.c so capacity requires valid accounting, at least one idle worker, and zero faulted workers. Run make test. Never use agentctl or the control socket through shell commands.' \
...
calls = [
    ...
    and event["item"].get("server") == "fractalos"
    and event["item"].get("tool") == "fractalos_pool_status"
]
if len(calls) != 1:
...
metric_lines = [
    line.removeprefix("FRACTALOS_CODEX_METRICS ")
    for line in pathlib.Path(sys.argv[2]).read_text().splitlines()
    if line.startswith("FRACTALOS_CODEX_METRICS ")
]
...
echo "PASS: official Codex queried live FractalOS, edited C, and passed tests"
