/*
 * virtio_blk.c — virtio-blk block device driver protection domain for FractalOS
 *
 ...
 * fractalos.system so that vaddr == paddr.  Without this, the physical address
...
/* log_drain_rings_vaddr required by log_drain_write() in fractalos.h */
...
static void virtio_blk_pd_notified(uint32_t ch)
{
    ...
    fractalos_log_channel("virtio_blk", ch);
}
