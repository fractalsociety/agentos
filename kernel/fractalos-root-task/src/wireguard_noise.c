...
int wg_noise_check_mac1(const uint8_t mac[WG_NOISE_MAC_LEN],
                        ...
                        const uint8_t local_static[WG_NOISE_KEY_LEN])
{
    return noise_check_mac1(mac, message, message_len, local_static);
}
...
{
    wg_blake2s_keyed(mac, WG_NOISE_MAC_LEN, cookie, WG_NOISE_COOKIE_LEN,
                     message, message_len);
}
...
int wg_noise_check_mac2(const uint8_t mac[WG_NOISE_MAC_LEN],
                        ...
                        const uint8_t cookie[WG_NOISE_COOKIE_LEN])
{
    uint8_t expected[WG_NOISE_MAC_LEN];
    wg_noise_write_mac2(expected, message, message_len, cookie);
    int valid = noise_equal(mac, expected, sizeof(expected));
    noise_zero(expected, sizeof(expected));
    return valid ? 0 : -1;
}
...
{
    wg_blake2s_keyed(cookie, WG_NOISE_COOKIE_LEN, secret, WG_NOISE_HASH_LEN,
                     addr, addr_len);
}

static void noise_cookie_key(uint8_t key[WG_NOISE_HASH_LEN],
                             const uint8_t responder_static[WG_NOISE_KEY_LEN])
{
    static const uint8_t label[8] = {'c','o','o','k','i','e','-','-'};
    uint8_t input[8u + WG_NOISE_KEY_LEN];
    noise_copy(input, label, sizeof(label));
    noise_copy(input + sizeof(label), responder_static, WG_NOISE_KEY_LEN);
    wg_blake2s(key, input, sizeof(input));
    noise_zero(input, sizeof(input));
}
...
{
    uint8_t key[WG_NOISE_HASH_LEN];
    if (receiver_index == 0u || nonce == (const uint8_t *)0
        || cookie == (const uint8_t *)0 || mac1 == (const uint8_t *)0
        || responder_static == (const uint8_t *)0)
        return -1;
    noise_zero(reply, WG_NOISE_COOKIE_REPLY_LEN);
    noise_store32(reply, 3u);
    noise_store32(reply + 4u, receiver_index);
    noise_copy(reply + 8u, nonce, WG_NOISE_COOKIE_NONCE_LEN);
    noise_cookie_key(key, responder_static);
    crypto_aead_lock(reply + 32u, reply + 48u, key, nonce, mac1,
                     WG_NOISE_MAC_LEN, cookie, WG_NOISE_COOKIE_LEN);
    noise_zero(key, sizeof(key));
    return 0;
}
...
{
    uint8_t key[WG_NOISE_HASH_LEN];
    if (noise_load32(reply) != 3u
        || last_mac1 == (const uint8_t *)0
        || responder_static == (const uint8_t *)0)
        return -1;
    noise_cookie_key(key, responder_static);
    if (crypto_aead_unlock(cookie_out, reply + 48u, key, reply + 8u,
                           last_mac1, WG_NOISE_MAC_LEN, reply + 32u,
                           WG_NOISE_COOKIE_LEN) != 0) {
        noise_zero(key, sizeof(key));
        return -1;
    }
    noise_zero(key, sizeof(key));
