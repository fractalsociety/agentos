/*
 * FractalOS USB Host Controller Service Protection Domain (usb_pd)
 *
 ...
 *
 * Build flag: -DFRACTALOS_USB_PD activates full logic.
...
static uint32_t h_list(sel4_badge_t b, const sel4_msg_t *req,
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        usb_strncpy((char *)dst[i].manufacturer, "FractalOS-USB", 32);
...
void usb_pd_main(seL4_CPtr my_ep, seL4_CPtr ns_ep)
{
    ...
    if (hw_present) {
        ...
        USB_LOG("USB host controller: stub mode (build with FRACTALOS_USB_PD and wire MMIO)\n");
    }
...
#else /* !FRACTALOS_USB_PD — no-op stub ────────────────────────────────────── */
