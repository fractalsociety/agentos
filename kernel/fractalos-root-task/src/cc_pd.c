/*
 * cc_pd.c — FractalOS Command-and-Control Protection Domain
 *
 * Receives binary-framed requests from external callers (agentctl,
 * fractalos_gui) via a VirtIO MMIO serial port, which QEMU bridges to
...
/* ─── Wire frame types ───────────────────────────────────────────────────── */
/*
 * These match the layout used by agentctl-ng and fractalos_gui exactly.
...
#elif defined(FRACTALOS_GUEST_LINUX)
...
#if defined(FRACTALOS_GUEST_LINUX) || defined(FRACTALOS_GUEST_FREEBSD)
 ...
 * (fractalos_gui) get well-formed responses and can display an empty state.
 */
 ...
 *
 * DECISION (fos-681): polecats track GENERIC AGENT work, not guest
 ...
 * fos-685).  Until then we relay through the placeholder slot below and,
 ...
 */
/* fos-7j5: PD_CNODE_SLOT_CONTROLLER_EP is defined in system_desc.h (slot
...
/*
 * MSG_CC_LOG_STREAM — drain a guest's serial output as ASCII bytes (fos-vsi).
 *
