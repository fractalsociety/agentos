...
int cap_broker_register(uint32_t owner_pd, fractalos_cap_desc_t cap, bool revokable) {
    for (int i = 0; i < MAX_CAPS; i++) {
 ...
 * On-disk path convention (for attest_verify.py): fractalos/attestation/<tick>.att
 *
...
/* Sentinel key_id in signature slot — identifies FractalOS in-kernel attester */
 ...
 *
 * TODO: poll AgentFS key fractalos/cap-policy.cbor when AgentFS IPC is wired
 */
