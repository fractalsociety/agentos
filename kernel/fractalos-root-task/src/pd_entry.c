/*
 * pd_entry.c — AArch64/RISC-V/x86-64 PD entry point for FractalOS service PDs
 *
 * Provides the _start symbol that the linker uses as the ELF entry point
 * (see tools/ld/fractalos.ld ENTRY(_start)).  This file is compiled into
 ...
 *
 * Placed in .text.start so the linker script (fractalos.ld) can guarantee it
