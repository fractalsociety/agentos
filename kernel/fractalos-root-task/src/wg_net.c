/*
 * FractalOS WireGuard Overlay Network Protection Domain — E5-S4: raw seL4 IPC
 *
 ...
 * define FRACTALOS_TEST_REAL_CRYPTO and link the production implementation. */
#ifdef FRACTALOS_TEST_REAL_CRYPTO
...
/* Host capture of the last OP_NET_WG_UDP_SEND for dataplane proofs. */
static uint32_t g_test_wg_udp_calls;
static uint32_t g_test_last_wg_udp_off;
static uint32_t g_test_last_wg_udp_len;
static uint32_t g_test_last_wg_udp_ip;
static uint32_t g_test_last_wg_udp_port;

/* sel4_call stub — records WireGuard UDP forwards; otherwise NET_OK. */
static inline void sel4_call(seL4_CPtr ep, const sel4_msg_t *req, sel4_msg_t *rep)
{
    (void)ep;
    for (int i = 0; i < 48; i++) rep->data[i] = 0;
    rep->opcode = 0; /* NET_OK / SEL4_ERR_OK */
    rep->length = 8;
    /* 0xBB == OP_NET_WG_UDP_SEND (net_server.h not yet included here). */
    if (req != (const sel4_msg_t *)0 && req->opcode == 0xBBu) {
        g_test_wg_udp_calls++;
        g_test_last_wg_udp_off = data_rd32(req->data, 0);
        g_test_last_wg_udp_len = data_rd32(req->data, 4);
        g_test_last_wg_udp_ip = data_rd32(req->data, 8);
        g_test_last_wg_udp_port = data_rd32(req->data, 12);
        data_wr32(rep->data, 0, 0u);
        data_wr32(rep->data, 4, g_test_last_wg_udp_len);
    }
}
...
static uint8_t     wg_cookie_secret[WG_NOISE_HASH_LEN];
static uint32_t    wg_cookie_secret_birth = 0u;
static bool        wg_cookie_secret_set = false;
static bool        wg_under_load = false;
static uint8_t     wg_drbg_key[WG_NOISE_HASH_LEN];
static uint64_t    wg_drbg_counter = 0u;
static bool        wg_drbg_ready = false;
...
static bool wg_pubkey_eq(const uint8_t *a, const uint8_t *b)
{
    uint8_t d = 0u;
...
static wg_peer_t *find_peer_by_pubkey(const uint8_t pubkey[WG_KEY_LEN])
{
    for (int i = 0; i < (int)WG_MAX_PEERS; i++) {
        if (peers[i].active && wg_pubkey_eq(peers[i].pubkey, pubkey))
            return &peers[i];
    }
    return NULL;
}

static void wg_peer_clear_session(wg_peer_t *p)
{
...
static void wg_peer_deactivate(wg_peer_t *p)
{
    wg_zero(p->pubkey, WG_KEY_LEN);
    wg_zero(p->preshared_key, WG_KEY_LEN);
    wg_peer_clear_session(p);
    wg_zero(p->last_timestamp, WG_NOISE_TIMESTAMP_LEN);
    wg_zero(p->latest_cookie, WG_NOISE_COOKIE_LEN);
    wg_zero(p->last_mac1_sent, WG_NOISE_MAC_LEN);
    p->cookie_valid = false;
    p->have_sent_mac1 = false;
    p->cookie_birth = 0u;
    p->active = false;
    p->peer_id = 0;
    p->last_handshake = 0;
...
static void wg_pack_cookie_addr(uint8_t out[6], uint32_t src_ip,
                                uint16_t src_port)
{
    out[0] = (uint8_t)(src_ip);
    out[1] = (uint8_t)(src_ip >> 8);
    out[2] = (uint8_t)(src_ip >> 16);
    out[3] = (uint8_t)(src_ip >> 24);
    out[4] = (uint8_t)(src_port >> 8);
    out[5] = (uint8_t)(src_port);
}

static int wg_cookie_fresh(const wg_peer_t *peer)
{
    if (!peer->cookie_valid) return 0;
    if (timer_tick < peer->cookie_birth) return 0;
    return (timer_tick - peer->cookie_birth)
        < (WG_COOKIE_SECRET_MAX_AGE - WG_COOKIE_SECRET_LATENCY);
}

static int wg_entropy_fill(uint8_t *out, uint32_t len);
static bool wg_index_in_use(uint32_t index, const wg_peer_t *except);

/* Under load: valid mac1 + invalid/missing mac2 → emit cookie reply (64).
 * Returns 1 if a cookie reply was emitted; 0 to continue Noise; -1 on error. */
static int wg_maybe_emit_cookie_reply(const uint8_t *message,
                                      uint32_t mac1_off,
                                      uint32_t src_ip, uint16_t src_port,
                                      uint32_t sender_index, sel4_msg_t *rep)
{
    uint8_t cookie[WG_NOISE_COOKIE_LEN];
    uint8_t nonce[WG_NOISE_COOKIE_NONCE_LEN];
    uint8_t reply[WG_NOISE_COOKIE_REPLY_LEN];
    uint8_t addr[6];

    if (!wg_under_load) return 0;
    if (!wg_cookie_secret_set) {
        data_wr32(rep->data, 0, WG_ERR_CRYPTO);
        rep->length = 4;
        return -1;
    }
    if (wg_noise_check_mac1(message + mac1_off, message, mac1_off,
                            wg_pubkey) != 0) {
        data_wr32(rep->data, 0, WG_ERR_CRYPTO);
        rep->length = 4;
        return -1;
    ...
    wg_pack_cookie_addr(addr, src_ip, src_port);
    wg_noise_compute_cookie(cookie, wg_cookie_secret, addr, sizeof(addr));
    if (wg_noise_check_mac2(message + mac1_off + WG_NOISE_MAC_LEN, message,
                            mac1_off + WG_NOISE_MAC_LEN, cookie) == 0) {
        wg_zero(cookie, sizeof(cookie));
        return 0; /* mac2 ok — continue handshake */
    }

    if (!wg_staging_range((uint32_t)WG_STAGING_COOKIE_NONCE_OFF,
                          WG_NOISE_COOKIE_NONCE_LEN, 0u,
    ...
    {
        uint32_t nz = 0u;
        for (uint32_t i = 0u; i < WG_NOISE_COOKIE_NONCE_LEN; i++)
            nz |= WG_STAGING[WG_STAGING_COOKIE_NONCE_OFF + i];
        if (nz == 0u) {
            if (wg_entropy_fill(nonce, sizeof(nonce)) != 0) {
                wg_zero(cookie, sizeof(cookie));
                data_wr32(rep->data, 0, WG_ERR_CRYPTO);
                rep->length = 4;
                return -1;
            }
            wg_copy_to_staging(WG_STAGING + WG_STAGING_COOKIE_NONCE_OFF, nonce,
                               sizeof(nonce));
        } else {
            wg_copy_from_staging(nonce, WG_STAGING + WG_STAGING_COOKIE_NONCE_OFF,
                                 sizeof(nonce));
        }
    }
    if (wg_noise_create_cookie_reply(reply, sender_index, nonce, cookie,
                                     message + mac1_off, wg_pubkey) != 0) {
        wg_zero(cookie, sizeof(cookie));
    ...
    wg_zero(cookie, sizeof(cookie));
    wg_zero(nonce, sizeof(nonce));
    wg_copy_to_staging(WG_STAGING + WG_STAGING_TX_OFF, reply, sizeof(reply));
    wg_zero(reply, sizeof(reply));
    data_wr32(rep->data, 0, WG_OK);
    data_wr32(rep->data, 4, (uint32_t)WG_STAGING_TX_OFF);
    data_wr32(rep->data, 8, WG_NOISE_COOKIE_REPLY_LEN);
    rep->length = 12;
    return 1;
}
...
/* Keyed-BLAKE2s counter DRBG. Seed via OP_WG_SEED_ENTROPY (KEY material). */
static int wg_entropy_fill(uint8_t *out, uint32_t len)
{
    uint32_t produced = 0u;
    if (!wg_drbg_ready || out == (uint8_t *)0 || len == 0u) return -1;
    while (produced < len) {
        uint8_t input[16];
        uint8_t block[WG_NOISE_HASH_LEN];
        uint32_t take;
        uint64_t ctr = wg_drbg_counter++;
        for (uint32_t i = 0u; i < 8u; i++)
            input[i] = (uint8_t)(ctr >> (8u * i));
        input[8] = 'w'; input[9] = 'g'; input[10] = '-'; input[11] = 'e';
        input[12] = 'n'; input[13] = 't'; input[14] = 'r'; input[15] = 'y';
        wg_blake2s_keyed(block, sizeof(block), wg_drbg_key, sizeof(wg_drbg_key),
                         input, sizeof(input));
        take = len - produced;
        if (take > sizeof(block)) take = (uint32_t)sizeof(block);
        for (uint32_t i = 0u; i < take; i++) out[produced + i] = block[i];
        wg_zero(block, sizeof(block));
        produced += take;
    }
    return 0;
}

static int wg_alloc_index(uint32_t *out, const wg_peer_t *except)
{
    for (int attempt = 0; attempt < 64; attempt++) {
        uint8_t rnd[4];
        uint32_t idx;
        if (wg_entropy_fill(rnd, sizeof(rnd)) != 0) return -1;
        idx = (uint32_t)rnd[0] | ((uint32_t)rnd[1] << 8)
            | ((uint32_t)rnd[2] << 16) | ((uint32_t)rnd[3] << 24);
        if (!wg_index_in_use(idx, except)) {
            *out = idx;
            return 0;
        }
    }
...
static void wg_make_timestamp(uint8_t out[WG_NOISE_TIMESTAMP_LEN])
{
    /* TAI64N: big-endian seconds with TAI64 epoch bit + nanoseconds. */
    uint64_t secs = 0x4000000000000025ULL + (uint64_t)timer_tick;
    uint32_t nanos = (timer_tick * 1000u) & 0x3fffffffu;
    out[0] = (uint8_t)(secs >> 56); out[1] = (uint8_t)(secs >> 48);
    out[2] = (uint8_t)(secs >> 40); out[3] = (uint8_t)(secs >> 32);
    out[4] = (uint8_t)(secs >> 24); out[5] = (uint8_t)(secs >> 16);
    out[6] = (uint8_t)(secs >> 8);  out[7] = (uint8_t)secs;
    out[8] = (uint8_t)(nanos >> 24); out[9] = (uint8_t)(nanos >> 16);
    out[10] = (uint8_t)(nanos >> 8); out[11] = (uint8_t)nanos;
}

static int wg_load_or_entropy_key(uint8_t out[WG_NOISE_KEY_LEN],
                                  uint32_t staging_off)
{
    if (staging_off == 0u) return wg_entropy_fill(out, WG_NOISE_KEY_LEN);
    if (!wg_staging_range(staging_off, WG_NOISE_KEY_LEN, 0u,
                          (uint32_t)WG_STAGING_TX_OFF))
        return -1;
    wg_copy_from_staging(out, WG_STAGING + staging_off, WG_NOISE_KEY_LEN);
    return 0;
}

static int wg_load_or_make_timestamp(uint8_t out[WG_NOISE_TIMESTAMP_LEN],
                                     uint32_t staging_off)
{
    if (staging_off == 0u) {
        wg_make_timestamp(out);
        return 0;
    }
    if (!wg_staging_range(staging_off, WG_NOISE_TIMESTAMP_LEN, 0u,
                          (uint32_t)WG_STAGING_TX_OFF))
        return -1;
    wg_copy_from_staging(out, WG_STAGING + staging_off, WG_NOISE_TIMESTAMP_LEN);
    return 0;
}

static int wg_resolve_sender_index(uint32_t *index, const wg_peer_t *except)
{
    if (*index == 0u) return wg_alloc_index(index, except);
    return wg_index_in_use(*index, except) ? -1 : 0;
}
...
/*
 * OP_WG_APPLY_NETMAP — sync peer table from a packed Headscale-style netmap.
 * Matching pubkeys roam endpoints without wiping the session.
 */
static uint32_t handle_apply_netmap(sel4_badge_t badge __attribute__((unused)),
                                    const sel4_msg_t *req, sel4_msg_t *rep,
                                    void *ctx __attribute__((unused)))
{
    uint32_t offset = data_rd32(req->data, 0);
    uint32_t nbytes = data_rd32(req->data, 4);
    uint32_t applied = 0u;
    uint32_t removed = 0u;
    uint32_t roamed = 0u;
    uint8_t keep[WG_MAX_PEERS];
    struct wg_netmap_header hdr;
    uint32_t i;

    for (i = 0u; i < WG_MAX_PEERS; i++)
        keep[i] = 0u;

    if (!wg_staging_vaddr || nbytes < WG_NETMAP_HEADER_BYTES
        || !wg_staging_range(offset, nbytes, 0u, WG_STAGING_SIZE)) {
        data_wr32(rep->data, 0, WG_ERR_CRYPTO);
        rep->length = 4;
        return SEL4_ERR_BAD_ARG;
    }

    wg_copy_from_staging((uint8_t *)&hdr, WG_STAGING + offset,
                         WG_NETMAP_HEADER_BYTES);
    if (hdr.version != WG_NETMAP_VERSION || hdr.peer_count > WG_MAX_PEERS) {
        data_wr32(rep->data, 0, WG_ERR_CRYPTO);
        rep->length = 4;
        return SEL4_ERR_BAD_ARG;
    }
    if (nbytes < WG_NETMAP_HEADER_BYTES
            + hdr.peer_count * WG_NETMAP_PEER_BYTES) {
        data_wr32(rep->data, 0, WG_ERR_CRYPTO);
        rep->length = 4;
        return SEL4_ERR_BAD_ARG;
    }

    for (i = 0u; i < hdr.peer_count; i++) {
        struct wg_netmap_peer entry;
        wg_peer_t *p;
        uint32_t entry_off = offset + WG_NETMAP_HEADER_BYTES
            + i * WG_NETMAP_PEER_BYTES;
        wg_copy_from_staging((uint8_t *)&entry, WG_STAGING + entry_off,
                             WG_NETMAP_PEER_BYTES);

        p = find_peer_by_pubkey(entry.pubkey);
        if (p != NULL) {
            if (p->endpoint_ip != entry.endpoint_ip
                || p->endpoint_port != entry.endpoint_port)
                roamed++;
            p->endpoint_ip = entry.endpoint_ip;
            p->endpoint_port = entry.endpoint_port;
            p->allowed_ip = entry.allowed_ip;
            p->allowed_mask = entry.allowed_mask;
            keep[p - peers] = 1u;
            applied++;
            continue;
        }

        {
            ...
            if (slot < 0) {
                data_wr32(rep->data, 0, WG_ERR_FULL);
                data_wr32(rep->data, 4, applied);
                data_wr32(rep->data, 8, removed);
                data_wr32(rep->data, 12, roamed);
                rep->length = 16;
                return SEL4_ERR_NO_MEM;
            }
            p = &peers[slot];
            new_id = (uint8_t)slot;
            for (uint8_t id = 0u; id < WG_MAX_PEERS; id++) {
            ...
            p->rx_bytes = 0;
            wg_zero(p->preshared_key, WG_KEY_LEN);
            for (uint32_t b = 0u; b < WG_KEY_LEN; b++)
                p->pubkey[b] = entry.pubkey[b];
            wg_noise_handshake_init(&p->handshake, p->pubkey, p->preshared_key);
            wg_peer_clear_session(p);
            wg_zero(p->last_timestamp, WG_NOISE_TIMESTAMP_LEN);
            p->last_handshake = 0;
    ...
    for (i = 0u; i < WG_MAX_PEERS; i++) {
        if (peers[i].active && keep[i] == 0u) {
            wg_peer_deactivate(&peers[i]);
            removed++;
        }
    }
...
static uint32_t handle_set_under_load(
    sel4_badge_t badge __attribute__((unused)), const sel4_msg_t *req,
    sel4_msg_t *rep, void *ctx __attribute__((unused)))
{
    wg_under_load = data_rd32(req->data, 0) != 0u;
    data_wr32(rep->data, 0, WG_OK);
    rep->length = 4;
    return SEL4_ERR_OK;
}

static uint32_t handle_set_cookie_secret(
    sel4_badge_t badge __attribute__((unused)), const sel4_msg_t *req,
    sel4_msg_t *rep, void *ctx __attribute__((unused)))
{
    uint32_t off = data_rd32(req->data, 0);
    if (!wg_staging_range(off, WG_NOISE_HASH_LEN, 0u,
                          (uint32_t)WG_STAGING_TX_OFF)) {
        data_wr32(rep->data, 0, WG_ERR_CRYPTO);
        rep->length = 4;
        return SEL4_ERR_BAD_ARG;
    }
    wg_copy_from_staging(wg_cookie_secret, WG_STAGING + off,
                         WG_NOISE_HASH_LEN);
    wg_cookie_secret_birth = timer_tick;
    wg_cookie_secret_set = true;
    data_wr32(rep->data, 0, WG_OK);
    rep->length = 4;
    return SEL4_ERR_OK;
}

static uint32_t handle_seed_entropy(
    sel4_badge_t badge __attribute__((unused)), const sel4_msg_t *req,
    sel4_msg_t *rep, void *ctx __attribute__((unused)))
{
    uint32_t off = data_rd32(req->data, 0);
    ...
    wg_copy_from_staging(wg_drbg_key, WG_STAGING + off, WG_NOISE_HASH_LEN);
    wg_drbg_counter = 1u;
    wg_drbg_ready = true;
    data_wr32(rep->data, 0, WG_OK);
    rep->length = 4;
...
/* Find active peer whose pubkey matches key[32]. Returns peer_id or 0xFF. */
static uint8_t wg_peer_id_by_pubkey(const uint8_t key[WG_KEY_LEN])
{
    wg_peer_t *p = find_peer_by_pubkey(key);
    if (p == NULL)
        return 0xFFu;
    return p->peer_id;
}
...
/*
 * OP_WG_DERP_WRAP — FrameSendPacket over peer dest pubkey + staging ciphertext.
 * Local stack buffer capped to staging TX max (host/target proof sizes).
 */
static uint32_t handle_derp_wrap(
    sel4_badge_t badge __attribute__((unused)), const sel4_msg_t *req,
    sel4_msg_t *rep, void *ctx __attribute__((unused)))
{
    uint32_t peer_id = data_rd32(req->data, 0);
    uint32_t ct_off = data_rd32(req->data, 4);
    uint32_t ct_len = data_rd32(req->data, 8);
    uint32_t out_off = data_rd32(req->data, 12);
    wg_peer_t *peer = find_peer((uint8_t)peer_id);
    uint8_t ct_buf[2048];
    uint8_t frame_buf[2048 + WG_DERP_HDR_LEN + WG_DERP_KEY_LEN];
    ...
    if (ct_len == 0u || ct_len > sizeof(ct_buf)
        || ct_len > WG_DERP_MAX_PACKET
        || !wg_staging_range(ct_off, ct_len, 0u, WG_STAGING_SIZE)
        || !wg_staging_range(out_off,
                             WG_DERP_HDR_LEN + WG_DERP_KEY_LEN + ct_len,
                             0u, WG_STAGING_SIZE)) {
        data_wr32(rep->data, 0, WG_ERR_CRYPTO);
        rep->length = 4;
        return SEL4_ERR_BAD_ARG;
    }
    wg_copy_from_staging(ct_buf, WG_STAGING + ct_off, ct_len);
    if (wg_derp_encode_send_packet(frame_buf, sizeof(frame_buf),
                                   peer->pubkey, ct_buf, ct_len,
                                   &frame_len) != 0) {
        data_wr32(rep->data, 0, WG_ERR_CRYPTO);
        rep->length = 4;
        return SEL4_ERR_INTERNAL;
    }
    wg_copy_to_staging(WG_STAGING + out_off, frame_buf, frame_len);
    data_wr32(rep->data, 0, WG_OK);
    data_wr32(rep->data, 4, out_off);
    data_wr32(rep->data, 8, frame_len);
    rep->length = 12;
    return SEL4_ERR_OK;
}

/* OP_WG_DERP_UNWRAP — parse Recv/Send/Forward; match remote pubkey to peer. */
static uint32_t handle_derp_unwrap(
    sel4_badge_t badge __attribute__((unused)), const sel4_msg_t *req,
    sel4_msg_t *rep, void *ctx __attribute__((unused)))
{
    uint32_t frame_off = data_rd32(req->data, 0);
    uint32_t frame_len = data_rd32(req->data, 4);
    uint32_t out_off = data_rd32(req->data, 8);
    uint8_t frame_buf[2048 + WG_DERP_HDR_LEN + (WG_DERP_KEY_LEN * 2u)];
    uint8_t type = 0u;
    uint32_t payload_len = 0u;
    const uint8_t *payload;
    const uint8_t *match_key = (const uint8_t *)0;
    const uint8_t *pkt = (const uint8_t *)0;
    uint32_t pkt_len = 0u;
    uint8_t peer_id;

    if (frame_len < WG_DERP_HDR_LEN || frame_len > sizeof(frame_buf)
        || !wg_staging_range(frame_off, frame_len, 0u, WG_STAGING_SIZE)) {
        data_wr32(rep->data, 0, WG_ERR_CRYPTO);
        rep->length = 4;
        return SEL4_ERR_BAD_ARG;
    }
    wg_copy_from_staging(frame_buf, WG_STAGING + frame_off, frame_len);
    if (wg_derp_parse_header(frame_buf, frame_len, &type, &payload_len) != 0
        || (WG_DERP_HDR_LEN + payload_len) != frame_len) {
        data_wr32(rep->data, 0, WG_ERR_CRYPTO);
        rep->length = 4;
        return SEL4_ERR_BAD_ARG;
    }
    payload = frame_buf + WG_DERP_HDR_LEN;

    if (type == WG_DERP_FRAME_RECV_PACKET) {
        const uint8_t *src = (const uint8_t *)0;
        if (wg_derp_decode_recv_packet(payload, payload_len, &src, &pkt,
                                       &pkt_len) != 0) {
            data_wr32(rep->data, 0, WG_ERR_CRYPTO);
            rep->length = 4;
            return SEL4_ERR_BAD_ARG;
        ...
        match_key = src;
    } else if (type == WG_DERP_FRAME_SEND_PACKET) {
        const uint8_t *dest = (const uint8_t *)0;
        if (wg_derp_decode_send_packet(payload, payload_len, &dest, &pkt,
                                       &pkt_len) != 0) {
            data_wr32(rep->data, 0, WG_ERR_CRYPTO);
            rep->length = 4;
            return SEL4_ERR_BAD_ARG;
        ...
        match_key = dest;
    } else if (type == WG_DERP_FRAME_FWD_PACKET) {
        const uint8_t *src = (const uint8_t *)0;
        const uint8_t *dest = (const uint8_t *)0;
        if (wg_derp_decode_forward_packet(payload, payload_len, &src, &dest,
                                          &pkt, &pkt_len) != 0) {
            data_wr32(rep->data, 0, WG_ERR_CRYPTO);
            rep->length = 4;
            return SEL4_ERR_BAD_ARG;
    ...
    peer_id = wg_peer_id_by_pubkey(match_key);
    if (peer_id == 0xFFu) {
        data_wr32(rep->data, 0, WG_ERR_NOPEER);
        rep->length = 4;
        return SEL4_ERR_NOT_FOUND;
    ...
    if (pkt_len == 0u
        || !wg_staging_range(out_off, pkt_len, 0u, WG_STAGING_SIZE)) {
        data_wr32(rep->data, 0, WG_ERR_CRYPTO);
        rep->length = 4;
        return SEL4_ERR_BAD_ARG;
    }
    wg_copy_to_staging(WG_STAGING + out_off, pkt, pkt_len);
    data_wr32(rep->data, 0, WG_OK);
    data_wr32(rep->data, 4, (uint32_t)peer_id);
    data_wr32(rep->data, 8, pkt_len);
    data_wr32(rep->data, 12, (uint32_t)type);
    rep->length = 16;
    return SEL4_ERR_OK;
}
...
static uint32_t handle_set_path_mode(
    sel4_badge_t badge __attribute__((unused)), const sel4_msg_t *req,
    sel4_msg_t *rep, void *ctx __attribute__((unused)))
{
    uint32_t peer_id = data_rd32(req->data, 0);
    uint32_t mode = data_rd32(req->data, 4);
    wg_peer_t *peer = find_peer((uint8_t)peer_id);
    ...
    if (mode != WG_PATH_DIRECT && mode != WG_PATH_DERP) {
        data_wr32(rep->data, 0, WG_ERR_CRYPTO);
        rep->length = 4;
        return SEL4_ERR_BAD_ARG;
    }
    peer->path_mode = (uint8_t)mode;
    data_wr32(rep->data, 0, WG_OK);
    rep->length = 4;
    return SEL4_ERR_OK;
}

static uint32_t handle_timer_tick(
    sel4_badge_t badge __attribute__((unused)),
    const sel4_msg_t *req __attribute__((unused)),
    sel4_msg_t *rep, void *ctx __attribute__((unused)))
{
    wg_net_timer_tick();
    data_wr32(rep->data, 0, WG_OK);
    data_wr32(rep->data, 4, timer_tick);
    rep->length = 8;
    return SEL4_ERR_OK;
}
    ...
    sel4_msg_t *rep, void *ctx __attribute__((unused)))
{
    ...
    if (wg_resolve_sender_index(&sender_index, peer) != 0
        || wg_load_or_entropy_key(ephemeral, ephemeral_off) != 0
        || wg_load_or_make_timestamp(timestamp, timestamp_off) != 0) {
    ...
    wg_zero(ephemeral, sizeof(ephemeral));
    wg_zero(timestamp, sizeof(timestamp));
    /* Remember mac1 for cookie-reply AAD; attach mac2 when cookie is fresh. */
    for (uint32_t i = 0u; i < WG_NOISE_MAC_LEN; i++)
        peer->last_mac1_sent[i] = message[116u + i];
    peer->have_sent_mac1 = true;
    if (wg_cookie_fresh(peer)) {
        wg_noise_write_mac2(message + 132u, message, 132u,
                            peer->latest_cookie);
    }
    ...
    data_wr32(rep->data, 12, sender_index);
    rep->length = 16;
    return SEL4_ERR_OK;
}
                              ...
                              void *ctx __attribute__((unused)))
{
    ...
    uint32_t sender_index = data_rd32(req->data, 12);
    uint32_t src_ip = data_rd32(req->data, 16);
    uint16_t src_port = (uint16_t)data_rd32(req->data, 20);
    ...
    if (type == 1u) {
        ...
        if (packet_len != WG_NOISE_INITIATION_LEN) {
            data_wr32(rep->data, 0, WG_ERR_CRYPTO);
            rep->length = 4;
            return SEL4_ERR_BAD_ARG;
        }
        wg_copy_from_staging(initiation, packet, sizeof(initiation));
        cookie_rc = wg_maybe_emit_cookie_reply(
            initiation, 116u, src_ip, src_port,
            data_rd32(initiation, 4), rep);
        if (cookie_rc < 0) {
        ...
        if (wg_resolve_sender_index(&sender_index, NULL) != 0
            || wg_load_or_entropy_key(ephemeral, ephemeral_off) != 0) {
            wg_zero(initiation, sizeof(initiation));
            data_wr32(rep->data, 0, WG_ERR_CRYPTO);
            rep->length = 4;
    ...
    if (type == 2u) {
        ...
        cookie_rc = wg_maybe_emit_cookie_reply(
            response, 60u, src_ip, src_port,
            data_rd32(response, 4), rep);
        if (cookie_rc < 0) {
            wg_zero(response, sizeof(response));
            return SEL4_ERR_PERM;
        }
    ...
    if (type == 3u) {
        uint8_t reply_msg[WG_NOISE_COOKIE_REPLY_LEN];
        uint8_t cookie[WG_NOISE_COOKIE_LEN];
        uint32_t receiver;
        wg_peer_t *peer;
        if (packet_len != WG_NOISE_COOKIE_REPLY_LEN) {
            data_wr32(rep->data, 0, WG_ERR_CRYPTO);
            rep->length = 4;
            return SEL4_ERR_BAD_ARG;
        }
        wg_copy_from_staging(reply_msg, packet, sizeof(reply_msg));
        receiver = data_rd32(reply_msg, 4);
        peer = find_peer_by_handshake_index(receiver);
        if (peer == NULL || !peer->have_sent_mac1
            || wg_noise_consume_cookie_reply(cookie, reply_msg,
                                            peer->last_mac1_sent,
                                            peer->pubkey) != 0) {
            wg_zero(reply_msg, sizeof(reply_msg));
            wg_zero(cookie, sizeof(cookie));
            data_wr32(rep->data, 0, WG_ERR_CRYPTO);
            rep->length = 4;
            return SEL4_ERR_PERM;
        ...
        wg_zero(reply_msg, sizeof(reply_msg));
        for (uint32_t i = 0u; i < WG_NOISE_COOKIE_LEN; i++)
            peer->latest_cookie[i] = cookie[i];
        wg_zero(cookie, sizeof(cookie));
        peer->cookie_valid = true;
        peer->cookie_birth = timer_tick;
        peer->have_sent_mac1 = false;
        data_wr32(rep->data, 0, WG_OK);
        data_wr32(rep->data, 4, peer->peer_id);
        data_wr32(rep->data, 8, WG_NOISE_COOKIE_LEN);
        rep->length = 12;
        return SEL4_ERR_OK;
    }
                              ...
                              void *ctx __attribute__((unused)))
{
    ...
    uint32_t pkt_len = WG_TRANSPORT_HDR_LEN + cipher_len;
    bool use_derp = (p->path_mode == WG_PATH_DERP) || (p->endpoint_ip == 0u);

    if (use_derp) {
        uint8_t ct_tmp[2048];
        uint8_t frame_buf[2048 + WG_DERP_HDR_LEN + WG_DERP_KEY_LEN];
        uint32_t frame_len = 0u;
        if (pkt_len > sizeof(ct_tmp)
            || !wg_staging_range((uint32_t)WG_STAGING_INGRESS_OFF,
                                WG_DERP_HDR_LEN + WG_DERP_KEY_LEN + pkt_len,
                                0u, WG_STAGING_SIZE)) {
            data_wr32(rep->data, 0, WG_ERR_CRYPTO);
            data_wr32(rep->data, 4, 0u);
            rep->length = 8;
        ...
        wg_copy_from_staging(ct_tmp, WG_STAGING + WG_STAGING_TX_OFF, pkt_len);
        if (wg_derp_encode_send_packet(frame_buf, sizeof(frame_buf),
                                       p->pubkey, ct_tmp, pkt_len,
                                       &frame_len) != 0) {
            data_wr32(rep->data, 0, WG_ERR_CRYPTO);
            data_wr32(rep->data, 4, 0u);
            rep->length = 8;
        ...
        wg_copy_to_staging(WG_STAGING + WG_STAGING_INGRESS_OFF, frame_buf,
                           frame_len);
        p->tx_bytes += pkt_len;
        data_wr32(rep->data, 0, WG_OK);
        data_wr32(rep->data, 4, cipher_len);
        data_wr32(rep->data, 8, frame_len);
        data_wr32(rep->data, 12, WG_PATH_DERP);
        rep->length = 16;
        return SEL4_ERR_OK;
    }
    ...
    data_wr32(rep->data, 4, cipher_len);
    data_wr32(rep->data, 8, pkt_len);
    data_wr32(rep->data, 12, WG_PATH_DIRECT);
    rep->length = 16;
    return SEL4_ERR_OK;
}
...
#else
    (void)rep;
    g_net_ep = 1;  /* enable host UDP forward capture via sel4_call stub */
#endif
...
static void wg_net_test_init(void)
{
    for (int i = 0; i < (int)WG_MAX_PEERS; i++) {
        ...
        wg_zero(peers[i].latest_cookie, WG_NOISE_COOKIE_LEN);
        wg_zero(peers[i].last_mac1_sent, WG_NOISE_MAC_LEN);
        peers[i].cookie_valid = false;
        peers[i].have_sent_mac1 = false;
        peers[i].cookie_birth = 0u;
    }
    active_peer_count = 0;
    ...
    wg_zero(wg_cookie_secret, WG_NOISE_HASH_LEN);
    wg_cookie_secret_birth = 0u;
    wg_cookie_secret_set = false;
    wg_under_load = false;
    wg_zero(wg_drbg_key, WG_NOISE_HASH_LEN);
    wg_drbg_counter = 0u;
    wg_drbg_ready = false;
    ...
    g_test_last_wg_udp_off = 0u;
    g_test_last_wg_udp_len = 0u;
    g_test_last_wg_udp_ip = 0u;
    g_test_last_wg_udp_port = 0u;
#endif
    ...
    sel4_server_register(&g_srv, OP_WG_APPLY_NETMAP, handle_apply_netmap, NULL);
    sel4_server_register(&g_srv, OP_WG_SET_UNDER_LOAD,
                         handle_set_under_load, NULL);
    sel4_server_register(&g_srv, OP_WG_SET_COOKIE_SECRET,
                         handle_set_cookie_secret, NULL);
    sel4_server_register(&g_srv, OP_WG_SEED_ENTROPY,
                         handle_seed_entropy, NULL);
    sel4_server_register(&g_srv, OP_WG_DERP_WRAP, handle_derp_wrap, NULL);
    sel4_server_register(&g_srv, OP_WG_DERP_UNWRAP, handle_derp_unwrap, NULL);
    sel4_server_register(&g_srv, OP_WG_SET_PATH_MODE,
                         handle_set_path_mode, NULL);
    sel4_server_register(&g_srv, OP_WG_TIMER_TICK, handle_timer_tick, NULL);
}
...
static void wg_net_timer_tick(void) {
    ...
    /* Rekey / reject-after-time (WireGuard §6.4) — fail closed on age. */
    for (int i = 0; i < (int)WG_MAX_PEERS; i++) {
        wg_peer_t *p = &peers[i];
        uint32_t age;
        if (!p->active || !p->session_established || p->last_handshake == 0u)
            continue;
        age = timer_tick - p->last_handshake;
        if (age >= WG_REJECT_AFTER_SECS || age >= WG_REKEY_AFTER_SECS) {
            log_drain_write(16, 16, "[wg_net] rekey/reject-after-time peer=");
            wg_log_dec(p->peer_id);
            log_drain_write(16, 16, " age=");
            wg_log_dec(age);
            log_drain_write(16, 16, "\n");
            wg_peer_clear_session(p);
            p->last_handshake = 0;
...
void pd_main(seL4_CPtr my_ep, seL4_CPtr ns_ep)
{
     ...
     * per-instance startup record at PD_STARTUP_RECORD_VA (fos-3ev).  An
