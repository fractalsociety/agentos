/*
 * FractalOS WASM3 Host — Implementation
 *
 * Bridges wasm3 interpreter with FractalOS swap slot protection domains.
...
struct wasm3_host {
    ...
    IM3Function     fn_heap_stats;   /* optional: __fractalos_heap_stats(i32*, i32*) */
    bool            initialized;
};
...
wasm3_host_t *wasm3_host_init(const uint8_t *wasm_bytes, uint32_t wasm_size) {
    ...
    /* Optional: __fractalos_heap_stats(live_allocs_ptr: i32, live_bytes_ptr: i32)
    ...
    result = m3_FindFunction(&host->fn_heap_stats, host->runtime, "__fractalos_heap_stats");
    if (result) {
 ...
 * WASM module's refcount_gc heap via the __fractalos_heap_stats export.
 *
 ...
 *
 * Returns false if the module does not export __fractalos_heap_stats (old modules
