/*
 * FractalOS Controller Protection Domain — raw seL4 IPC (E5-S2)
 *
...
#include "contracts/agent_pool_contract.h"  /* agent_pool_occupancy (fos-7j5) */
...
/* Stub fractalos_log_boot */
static inline void fractalos_log_boot(const char *n) { (void)n; }
...
#define FRACTALOS_CRIT(s) do {} while(0)
#endif
...
#define FRACTALOS_UART_VA  0x10001000UL

static void ctrl_puts(const char *s)
{
#ifndef FRACTALOS_TEST_HOST
    volatile uint32_t *fr = (volatile uint32_t *)(FRACTALOS_UART_VA + 0x18u);
    volatile uint32_t *dr = (volatile uint32_t *)(FRACTALOS_UART_VA + 0x00u);
...
static void demo_sequence(void)
{
    ...
    if (rc == SEL4_ERR_OK) {
        ...
        ctrl_puts("...\n[controller] Object payload: 'Hello from FractalOS' (18 bytes)\n");
    } else {
                                           ...
                                           void *ctx)
{
    ...
    if (!vmm_vcpu_regs_vaddr) {
        FRACTALOS_CRIT("[cap_policy] vCPU regs shmem not mapped — EPERM");
        rep->data[0] = 0u;
        rep->length  = 1u;
...
#endif
        if (el_rc != 0) {
            FRACTALOS_CRIT("[cap_policy] vCPU EL2/CPL0 escalation REJECTED");
            rep->data[0] = 0u;
            rep->length  = 1u;
            return SEL4_ERR_PERM;
        }
...
/*
 * handle_agentpool_status (fos-7j5 / fos-681)
 *
...
void controller_main(seL4_CPtr my_ep, seL4_CPtr ns_ep)
{
    ...
    ctrl_puts("[controller] Initializing FractalOS core services\n");
    ...
    {
        char count[12];
        ctrl_puts("FRACTALOS_BOOT_HEALTH ns_registered=");
    ...
    ctrl_puts("[controller] *** FractalOS controller boot complete ***\n");
    ctrl_puts("[controller] Ready for agents.\n");
    /* Canonical marker follows the boot-health gate. */
    ctrl_puts("FractalOS boot complete\n");
