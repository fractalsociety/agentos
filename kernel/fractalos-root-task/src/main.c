/*
 * main.c — FractalOS root task boot sequence
 *
...
#include "fractalos.h"         /* sel4_dbg_puts                                    */
 ...
 *
 * Most FractalOS PDs are short-running IPC servers, so the conservative 1% CPU
 ...
 * bundle data (fractalos_img_hdr_t + PD entry table + PD ELFs) and objcopy
...
/* fractalos.img header/entry types for the embedded bundle */
#define FRACTALOS_IMAGE_MAGIC_BUNDLE  UINT64_C(0x4652414354414C4F)
...
typedef struct {
    ...
    uint8_t  _pad[28];
} __attribute__((packed)) fractalos_bundle_hdr_t;
...
typedef struct {
    ...
    uint8_t  priority;
    uint8_t  _pad[7];
} __attribute__((packed)) fractalos_bundle_pd_entry_t;

/*
 * bundle_name_match — compare a NUL-terminated elf_path against the
 * fixed-width (48-byte) name field of an fractalos_bundle_pd_entry_t.
 *
...
{
    if (bundle_size() < sizeof(fractalos_bundle_hdr_t)) {
        return (const void *)0;
    }

    const uint8_t *bundle = __pd_bundle_start;
    const fractalos_bundle_hdr_t *hdr = (const fractalos_bundle_hdr_t *)bundle;

    if (hdr->magic != FRACTALOS_IMAGE_MAGIC_BUNDLE) {
        return (const void *)0;
    }

    const fractalos_bundle_pd_entry_t *table =
        (const fractalos_bundle_pd_entry_t *)(bundle + hdr->pd_table_off);
...
static const void *boot_find_elf(const seL4_BootInfo *bi, const char *elf_path)
{
    ...
    while (p + sizeof(seL4_BootInfoHeader) <= end) {
        ...
        if (hdr->id == FRACTALOS_BOOTINFO_HEADER_ELF &&
            hdr->len >= sizeof(seL4_BootInfoHeader) + sizeof(fractalos_elf_region_t)) {

            const fractalos_elf_region_t *er =
                (const fractalos_elf_region_t *)(p + sizeof(seL4_BootInfoHeader));
            ...
            if (match) {
                /*
                 * ELF data starts immediately after the fractalos_elf_region_t
...
/*
 * PL011 UART mapped at FRACTALOS_UART_VA in the root task's VSpace.
 ...
 */
#define FRACTALOS_UART_PA  0x09000000UL  /* PL011 UART0 physical address on QEMU virt */
#define FRACTALOS_UART_VA  0x10001000UL  /* VA in root + controller VSpaces           */
...
#define EXEC_TRANSPORT_MMIO_PAGE_PA 0x0A001000UL /* QEMU virtio-mmio bus.8 */
#define EXEC_TRANSPORT_VIRTIO_VA   0x10009000UL
#define EXEC_TRANSPORT_STARTUP_VA  0x1000A000UL
#define MCP_TRANSPORT_MMIO_PAGE_PA 0x0A002000UL /* QEMU virtio-mmio bus.16 */
#define MCP_TRANSPORT_VIRTIO_VA    0x1000B000UL
#define MCP_TRANSPORT_STARTUP_VA   0x1000C000UL
...
static seL4_CPtr g_exec_transport_mmio_frame_cap = seL4_CapNull;
static seL4_CPtr g_mcp_transport_mmio_frame_cap = seL4_CapNull;
...
#define TOOLSVC_INTERNAL_ARENA_PAGES (TOOLSVC_INTERNAL_ARENA_SIZE / 4096u)
 ...
 * record (fos-3ev): swap_slot[_N], app_slot[_N], wg_net, vibe_swap.
 */
...
void root_task_main(const seL4_BootInfo *bi)
{
     ...
     * frame cap in the root task's CNode, then map it at FRACTALOS_UART_VA in
     ...
     */
    {
        seL4_Error uart_err = ut_alloc_device_cap(FRACTALOS_UART_PA, &g_uart_frame_cap);
        if (uart_err == seL4_NoError) {
            uart_err = pd_vspace_map_device_frame(seL4_CapInitThreadVSpace,
                                                   g_uart_frame_cap,
                                                   FRACTALOS_UART_VA);
            if (uart_err == seL4_NoError) {
                g_uart_dr = (volatile uint32_t *)(FRACTALOS_UART_VA + 0x00u);
                g_uart_fr = (volatile uint32_t *)(FRACTALOS_UART_VA + 0x18u);
            }
        }
    }
    ...
    {
        seL4_Error exec_transport_err = ut_alloc_device_cap(
            EXEC_TRANSPORT_MMIO_PAGE_PA, &g_exec_transport_mmio_frame_cap);
        dbg_puts("[rt] exec transport virtio-mmio frame cap err=");
        dbg_hex((seL4_Word)exec_transport_err);
        dbg_puts(" cap=");
        dbg_hex((seL4_Word)g_exec_transport_mmio_frame_cap);
        dbg_puts("\n");
    }

    {
        seL4_Error mcp_transport_err = ut_alloc_device_cap(
            MCP_TRANSPORT_MMIO_PAGE_PA, &g_mcp_transport_mmio_frame_cap);
        dbg_puts("[rt] mcp transport virtio-mmio frame cap err=");
        dbg_hex((seL4_Word)mcp_transport_err);
        dbg_puts(" cap=");
        dbg_hex((seL4_Word)g_mcp_transport_mmio_frame_cap);
        dbg_puts("\n");
    }
    ...
    for (uint32_t i = 0u; i < sys->pd_count; i++) {
         ...
         * output at FRACTALOS_UART_VA.  We copy the root task's UART frame cap
        ...
        /* The execution bridge uses bus.8, whose MMIO registers occupy a
         * separate 4 KiB physical frame. Only exec_transport receives that
         * frame capability; workers and ExecSvc receive endpoint caps only. */
        if (name_eq(pd->name, "exec_transport")) {
            seL4_CPtr et_virtio_cap = seL4_CapNull;
            seL4_Error ve = seL4_InvalidCapability;
            if (g_exec_transport_mmio_frame_cap != seL4_CapNull) {
                seL4_Word slot = ut_alloc_slot();
                if (slot != seL4_CapNull) {
                    ve = seL4_CNode_Copy(
                        seL4_CapInitThreadCNode, slot, 64u,
                        seL4_CapInitThreadCNode,
                        g_exec_transport_mmio_frame_cap, 64u,
                        seL4_AllRights);
                    if (ve == seL4_NoError) et_virtio_cap = (seL4_CPtr)slot;
                } else {
                    ve = seL4_NotEnoughMemory;
                }
            }
            if (ve == seL4_NoError && et_virtio_cap != seL4_CapNull) {
                ve = pd_vspace_map_device_frame(vspace, et_virtio_cap,
                                                EXEC_TRANSPORT_VIRTIO_VA);
            }
            dbg_puts("[rt] exec_transport VirtIO page err=");
            dbg_hex((seL4_Word)ve);
            dbg_puts("\n");
            ...
            for (uint32_t page = 0u; page < 3u; page++) {
                ...
                dbg_puts("[rt] exec_transport vq pa=");
                dbg_hex(vq_pas[page]);
                dbg_puts(" err=");
                dbg_hex((seL4_Word)ve);
                dbg_puts("\n");
            ...
            if (ve == seL4_NoError) {
                ...
                if (ve == seL4_NoError) {
                    ...
                    seL4_ARCH_Page_Unmap(startup_cap);
                    ve = pd_vspace_map_device_frame(vspace, startup_cap,
                                                    EXEC_TRANSPORT_STARTUP_VA);
                }
            }
            dbg_puts("[rt] exec_transport startup map err=");
            dbg_hex((seL4_Word)ve);
            dbg_puts("\n");
        }

        /* External MCP uses bus.16 on its own physical MMIO page. Only the
         * MCP transport PD receives the page; ToolSvc receives an endpoint
         * and a service-private staging mapping, while workers receive neither. */
        if (name_eq(pd->name, "mcp_transport")) {
            seL4_CPtr mp_virtio_cap = seL4_CapNull;
            seL4_Error ve = seL4_InvalidCapability;
            if (g_mcp_transport_mmio_frame_cap != seL4_CapNull) {
                seL4_Word slot = ut_alloc_slot();
                if (slot != seL4_CapNull) {
                    ve = seL4_CNode_Copy(
                        seL4_CapInitThreadCNode, slot, 64u,
                        seL4_CapInitThreadCNode,
                        g_mcp_transport_mmio_frame_cap, 64u,
                        seL4_AllRights);
                    if (ve == seL4_NoError) mp_virtio_cap = (seL4_CPtr)slot;
                } else {
                    ve = seL4_NotEnoughMemory;
                }
            }
            if (ve == seL4_NoError && mp_virtio_cap != seL4_CapNull) {
                ve = pd_vspace_map_device_frame(vspace, mp_virtio_cap,
                                                MCP_TRANSPORT_VIRTIO_VA);
            }
            dbg_puts("[rt] mcp_transport VirtIO page err=");
            dbg_hex((seL4_Word)ve);
            dbg_puts("\n");
            ...
            for (uint32_t page = 0u; page < 3u; page++) {
                ...
                dbg_puts("[rt] mcp_transport vq pa=");
                dbg_hex(vq_pas[page]);
                dbg_puts(" err=");
                dbg_hex((seL4_Word)ve);
                dbg_puts("\n");
            ...
            if (ve == seL4_NoError) {
                ...
                if (ve == seL4_NoError) {
                    ...
                    seL4_ARCH_Page_Unmap(startup_cap);
                    ve = pd_vspace_map_device_frame(vspace, startup_cap,
                                                    MCP_TRANSPORT_STARTUP_VA);
                }
            }
            dbg_puts("[rt] mcp_transport startup map err=");
            dbg_hex((seL4_Word)ve);
            dbg_puts("\n");
        }

#ifdef FRACTALOS_SEL4_TEST_IMAGE
        /* fos-8f5: map UART0 into the contract-runner PD so it can emit TAP
        ...
        /* ── 4g.4.9: EventBus ring RAM region (fos-gom) ─────────────────
        ...
        if (name_eq(pd->name, "tool_svc")) {
            ...
            dbg_puts("\n");
        } else if (name_eq(pd->name, "mcp_transport")) {
            seL4_Error te = pd_vspace_map_shared_pages(
                vspace,
                (seL4_Word)TOOLSVC_SHMEM_VADDR
                    + TOOLSVC_INTERNAL_ARENA_OFFSET,
                TOOLSVC_INTERNAL_ARENA_SIZE, 1,
                &g_toolsvc_shmem_frames[
                    TOOLSVC_INTERNAL_ARENA_OFFSET / 4096u],
                TOOLSVC_INTERNAL_ARENA_PAGES);
            dbg_puts("[rt] ToolSvc private MCP transport arena map err=");
            dbg_hex((seL4_Word)te);
            dbg_puts("\n");
        ...
        /* AgentFS owns mutable workspace state once at system scope. MemoryCap
         * clients map only their badge-selected transfer/overlay window. */
        if (name_eq(pd->name, "agentfs")) {
            seL4_Error ae = pd_vspace_map_shared_pages(
                vspace, (seL4_Word)AGENTFS_SHMEM_VADDR,
                AGENTFS_SHMEM_SIZE, 1,
                g_agentfs_shmem_frames, AGENTFS_SHMEM_PAGES);
            dbg_puts("[rt] AgentFS shared arena map err=");
            dbg_hex((seL4_Word)ae);
            dbg_puts("\n");
        } else if (pd_has_init_service(pd, SVC_ID_AGENTFS)
                   && i < AGENTFS_CLIENT_SLOT_COUNT) {
            uint32_t client_offset = AGENTFS_CLIENT_ARENA_OFFSET(i);
            seL4_Error ae = pd_vspace_map_shared_pages(
                vspace,
                (seL4_Word)AGENTFS_SHMEM_VADDR + client_offset,
                AGENTFS_CLIENT_ARENA_SIZE, 1,
                &g_agentfs_shmem_frames[client_offset / 4096u],
                AGENTFS_CLIENT_ARENA_PAGES);
            dbg_puts("[rt] AgentFS client partition map pd=");
            dbg_puts(pd->name);
            dbg_puts(" err=");
            dbg_hex((seL4_Word)ae);
            dbg_puts("\n");
        ...
        /* ExecSvc and its dedicated transport share the service-private
         * arena. An ExecCap client maps only its badge-selected window. */
        if (name_eq(pd->name, "exec_verify")
            || name_eq(pd->name, "exec_transport")) {
        ...
        /* ── 4g.4.8: Startup record for parameterized PDs (fos-3ev) ────── */
        /*
        ...
        /* ── 4g.6: Allocate MCS reply object at slot FRACTALOS_IPC_REPLY_CAP ── */
        /*
         ...
         * (FRACTALOS_IPC_REPLY_CAP) for this purpose in every service PD's CNode.
...
#ifdef CONFIG_KERNEL_MCS
        {
            ...
            if (reply_slot != seL4_CapNull) {
                ...
                if (re == seL4_NoError) {
                    re = seL4_CNode_Copy(
                             pd_cnode,              /* dest CNode */
                             9u,                    /* dest slot (FRACTALOS_IPC_REPLY_CAP) */
                             (seL4_Word)pd->cnode_size_bits,
...
#if defined(FRACTALOS_SEL4_TEST_IMAGE) && defined(__aarch64__)
    /* fos-8f5: on aarch64 the contract-runner PD (test_runner.elf) emits the
...
#elif defined(FRACTALOS_SEL4_TEST_IMAGE)
