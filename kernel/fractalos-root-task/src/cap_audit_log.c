/*
 * FractalOS Capability Audit Log Protection Domain
 *
...
static void cap_audit_log_pd_init(void) {
    fractalos_log_boot("cap_audit_log");
    cap_audit_init();
...
void pd_main(seL4_CPtr my_ep, seL4_CPtr ns_ep) { cap_audit_log_main(my_ep, ns_ep); }
