...
/* fos-8f5: a target contract-runner PD is appended only in test images. */
...
/* CC init-ep counts include the fos-7j5 controller endpoint (+1). */
#if defined(FRACTALOS_FAULT_INJECT) && defined(FRACTALOS_GUEST_BOTH)
#define AOS_AARCH64_PD_COUNT (29u + AOS_TEST_PD_EXTRA)
#define AOS_CC_INIT_EP_COUNT 7u
#elif defined(FRACTALOS_FAULT_INJECT)
#define AOS_AARCH64_PD_COUNT (28u + AOS_TEST_PD_EXTRA)
#define AOS_CC_INIT_EP_COUNT 7u
#elif defined(FRACTALOS_GUEST_BOTH)
#define AOS_AARCH64_PD_COUNT (28u + AOS_TEST_PD_EXTRA)
...
#elif defined(FRACTALOS_GUEST_LINUX) || defined(FRACTALOS_GUEST_FREEBSD)
...
const system_desc_t system_desc_aarch64 = {
    ...
    .pds = {
         ...
         * service it is waiting for can preempt and respond. */
        {
            ...
            .cnode_size_bits = 10u,
            .priority       = 50u,
            /* fos-7j5: expose the controller's inbound server endpoint so
         ...
         */
        {
#if defined(FRACTALOS_GUEST_FREEBSD) && !defined(FRACTALOS_GUEST_BOTH)
            .name           = "freebsd_vmm",
         ...
         * controller (50) but below all the services it calls. */
        {
            ...
            .init_eps = {
                { SVC_ID_NAMESERVER, PD_CNODE_SLOT_NAMESERVER_EP },
                { SVC_ID_LOG_DRAIN,  PD_CNODE_SLOT_LOG_DRAIN_EP  },
#if defined(FRACTALOS_GUEST_LINUX)
                { SVC_ID_LINUX_VMM,  PD_CNODE_SLOT_LINUX_VMM_EP  },
#endif
#if defined(FRACTALOS_GUEST_FREEBSD)
         ...
         * below vibe_engine (165) and other providers it calls. */
        {
            ...
            .init_eps = {
                ...
                { SVC_ID_VM_MANAGER,  PD_CNODE_SLOT_VM_MANAGER_EP  },
                /* fos-7j5: controller EP for live MSG_AGENTPOOL_STATUS. */
                { SVC_ID_CONTROLLER,  PD_CNODE_SLOT_CONTROLLER_EP,
                  CONTROLLER_RIGHT_AGENT_TASK },
#if defined(FRACTALOS_FAULT_INJECT)
...
#ifdef FRACTALOS_SEL4_TEST_IMAGE
        /* fos-8f5 / fos-0h4: on-target contract TAP runner.  Test image
         ...
         * cnode_size_bits=9 (512 slots) covers both layouts. */
        {
             ...
             * the ring-unmapped state (fos-gom). */
            .priority       = 250u,
            .self_svc_id    = 0u,
            .init_ep_count  = 17u,
            .init_eps = {
