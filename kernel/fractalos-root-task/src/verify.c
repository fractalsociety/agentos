/*
 * verify.c — Ed25519 + SHA-256 WASM module verifier for FractalOS (seL4/Microkit)
 *
 ...
 *
 * We do NOT include fractalos.h here because it transitively pulls in
...
/* log_drain_write is declared in fractalos.h (included above). */
...
#define FRACTALOS_SIG_SECTION_NAME   "fractalos.signature"
#define FRACTALOS_SIG_SECTION_NAMELEN 17u
...
/* Find the "fractalos.signature" custom section (id == 0x00).
 *
                                   ...
                                   const uint8_t **out_payload)
{
    ...
    while (pos < len) {
        ...
        if (sec_id == 0x00) {
            ...
            if (name_len == FRACTALOS_SIG_SECTION_NAMELEN &&
                    ...
                    <= body_start + sec_size)
            {
                ...
                if (memcmp(name_start, FRACTALOS_SIG_SECTION_NAME,
                           FRACTALOS_SIG_SECTION_NAMELEN) == 0)
                {
                    *out_off       = pos;
                    *out_sec_total = 1 + leb_bytes + (size_t)sec_size;
                    *out_payload   = name_start + FRACTALOS_SIG_SECTION_NAMELEN;
                    return true;
                }
            }
        }
 ...
 * the 128-byte fractalos.signature payload.
 */
...
/*
 * verify_capabilities_manifest - verify that the fractalos.capabilities
 * custom section hash matches the fractalos.cap_signature section.
 *
 * The signing model:
 *   fractalos.cap_signature = SHA-256(fractalos.capabilities section bytes)
 *   signed with the agent issuer's ED25519 key (stored in fractalos.signature)
 *
...
int verify_capabilities_manifest(const uint8_t *wasm, size_t wasm_len)
{
    ...
    /* 1. Find fractalos.capabilities section */
    const uint8_t *caps_body = find_wasm_custom_section(
        wasm, wasm_len, "fractalos.capabilities", &caps_len);
    if (!caps_body) return -1;

    /* 2. Find fractalos.cap_signature section (must be exactly 32 bytes) */
    const uint8_t *cap_sig = find_wasm_custom_section(
        wasm, wasm_len, "fractalos.cap_signature", &sig_len);
                        ...
                        const uint8_t *trusted_pubkey)
{
    ...
    if (!wasm_find_sig_section(wasm, len, &sig_off, &sig_sec_total, &payload)) {
        log_drain_write(15, 15, "[vibe_verify] WARNING: no fractalos.signature section found\n");
#if VIBE_VERIFY_MODE
        return false;
#else
        return true;
