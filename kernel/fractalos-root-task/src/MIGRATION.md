# FractalOS PD Migration Template: Microkit to Raw seL4 IPC
...
```c
#include "fractalos.h"   /* pulls in microkit.h and Microkit ABI */
```
...
For PDs that need contract opcodes but no longer pull in `fractalos.h`,
...
#ifdef FRACTALOS_TEST_HOST
#include "framework.h"
#define FRACTALOS_TEST_HOST  /* ensure PD source sees the flag */
#include "../../kernel/fractalos-root-task/src/<pd>.c"
...
|----------------------------------|--------------------------------------------|
| `#include "fractalos.h"`           | `#include "sel4_ipc.h"` + `sel4_server.h`  |
...
1. `kernel/fractalos-root-task/src/<pd>.c` — replace Microkit callbacks with
   `sel4_server_t` dispatch and `<pd>_main` entry point.
2. `tests/api/test_<pd>.c` — new file, TAP tests covering every opcode.
3. `kernel/fractalos-root-task/src/MIGRATION.md` — this file (updated once, not per PD).
