/*
 * FractalOS Power Manager Protection Domain
 *
 ...
 *
 * Build flag: -DFRACTALOS_POWER_MGR activates full logic; without it the PD
...
#ifdef FRACTALOS_POWER_MGR_TP_WIRED
    if (g_tp_ep != 0) {
...
#else /* !FRACTALOS_POWER_MGR — no-op stub ──────────────────────────────── */

void power_mgr_main(seL4_CPtr my_ep, seL4_CPtr ns_ep)
{
    (void)my_ep; (void)ns_ep;
    sel4_dbg_puts("[power_mgr] stub (build with FRACTALOS_POWER_MGR=1 to enable)\n");
