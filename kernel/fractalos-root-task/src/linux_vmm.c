/*
 * FractalOS Linux VMM — Virtual Machine Monitor
 *
 * Boots a Linux guest inside FractalOS using libvmm on seL4/Microkit.
 * The VMM bridges IPC between native FractalOS agents (via controller)
 * and the Linux guest, enabling agents to run on a full Linux userland
 * while remaining under seL4 capability isolation.
 *
 * Based on au-ts/libvmm examples/simple, extended with FractalOS IPC.
 *
 * guest_contract.h compliance (Phase 3d):
 *   This VMM implements the FractalOS guest binding protocol defined in
...
/* Raw FractalOS CNode layout constants.
 *
 ...
 */
#define FRACTALOS_IRQ_CAP_BASE     64u
#define FRACTALOS_VMM_TCB_CAP_BASE 266u
#define FRACTALOS_VMM_VCPU_CAP_BASE 330u

/* ── Microkit shim ───────────────────────────────────────────────────────
 *
 * libvmm was designed as a Microkit library and references several Microkit
 * runtime symbols for debug output and channel state.  FractalOS does not use
 ...
 * FractalOS linux_vmm does not use the Microkit runtime; these are zero-valued
...
#if defined(FRACTALOS_GUEST_BOTH)
 ...
 * headroom. Native FractalOS workers do not use this VM allocation. */
#if defined(FRACTALOS_GUEST_CODEX)
...
#elif defined(FRACTALOS_GUEST_CODEX)
 ...
 * Active only in linux_vmm_test.system; in full FractalOS.system id=6 is absent
 ...
 * FRACTALOS_IRQ_CAP_BASE + irq_index.  system_desc_aarch64.c orders them as
 * virtio-net, virtio-blk0, virtio-blk1.
 */
static const seL4_CPtr g_virtio_net_irq_cap =
    (seL4_CPtr)(FRACTALOS_IRQ_CAP_BASE + 0u);
static const seL4_CPtr g_virtio_blk_irq_cap =
    (seL4_CPtr)(FRACTALOS_IRQ_CAP_BASE + 1u);
static const seL4_CPtr g_virtio_blk2_irq_cap =
    (seL4_CPtr)(FRACTALOS_IRQ_CAP_BASE + 2u);
static const seL4_CPtr g_uart_irq_cap =
    (seL4_CPtr)(FRACTALOS_IRQ_CAP_BASE + 3u);
...
static void linux_vmm_suspend_guest_tcb(void)
{
    ...
    seL4_Error err = seL4_TCB_ReadRegisters(
        (seL4_CPtr)(FRACTALOS_VMM_TCB_CAP_BASE + GUEST_BOOT_VCPU_ID),
        true,
        0,
    ...
    if (err != seL4_NoError) {
        ...
        seL4_TCB_Suspend((seL4_CPtr)(FRACTALOS_VMM_TCB_CAP_BASE + GUEST_BOOT_VCPU_ID));
    }
}
...
static seL4_MessageInfo_t linux_vmm_rpc(seL4_MessageInfo_t info)
{
    ...
    case MSG_GUEST_RESUME: {
        ...
        if (g_guest_state == GUEST_STATE_SUSPENDED) {
            seL4_TCB_Resume((seL4_CPtr)(FRACTALOS_VMM_TCB_CAP_BASE + GUEST_BOOT_VCPU_ID));
        }
        g_guest_state = GUEST_STATE_RUNNING;
        rep.opcode = GUEST_OK;
        break;
...
void init(void)
{
    ...
    LOG_VMM("FractalOS linux_vmm starting \"linux_vmm\"\n");
    ...
    vmm_register_vcpu(GUEST_BOOT_VCPU_ID,
                      FRACTALOS_VMM_VCPU_CAP_BASE + GUEST_BOOT_VCPU_ID,
                      FRACTALOS_VMM_TCB_CAP_BASE  + GUEST_BOOT_VCPU_ID);
     ...
     * In the full FractalOS system serial_pd owns IRQ 33; id=6 is absent and
...
/*
 * FractalOS root-task CNode layout:
 *   my_ep:             passed in x0 by pd_entry.c
 *   FRACTALOS_IPC_REPLY_CAP: reserved MCS reply object slot
 */
...
{
    (void)ns_ep;
    linux_vmm_main(my_ep, FRACTALOS_IPC_REPLY_CAP);
}
