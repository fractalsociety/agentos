 ...
 *
 * FRACTALOS_TEST_HOST: stubs return seL4_IllegalOperation so the host-side
...
#include "boot_info.h"
#include <stdint.h>
#if defined(__x86_64__) && !defined(FRACTALOS_TEST_HOST)
...
{
    if (backing_frames == (seL4_CPtr *)0 || size == 0u
        || (va_start & (PAGE_SIZE - 1u)) != 0u
        || (size & (PAGE_SIZE - 1u)) != 0u
        || backing_frame_count != size / PAGE_SIZE)
        return seL4_InvalidArgument;

    seL4_CapRights_t rights = writable ? seL4_AllRights : seL4_CanRead;
    for (size_t i = 0u; i < backing_frame_count; i++) {
        if (backing_frames[i] == seL4_CapNull) {
            seL4_Error err = ut_alloc_cap(seL4_ARM_SmallPageObject, 0u,
                                          &backing_frames[i]);
            if (err != seL4_NoError) return err;
        }
        ...
        if (err != seL4_NoError) return err;
        err = map_page((seL4_CPtr)copy_slot, vspace,
                       va_start + (seL4_Word)i * PAGE_SIZE,
                       rights, seL4_ARM_Default_VMAttributes);
        if (err != seL4_NoError) return err;
    }
    return seL4_NoError;
