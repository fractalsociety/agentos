#
# FractalOS Linux VMM Build — Sub-Makefile
#
...
#   BUILD_DIR, FRACTALOS_ARCH, FRACTALOS_BOARD
#

# ─── Paths ────────────────────────────────────────────────────────────────
KERNEL_SRC_DIR := $(abspath $(dir $(lastword $(MAKEFILE_LIST))))
FRACTALOS_ROOT   := $(abspath $(KERNEL_SRC_DIR)/../..)
LIBVMM_ABS     := $(FRACTALOS_ROOT)/libvmm
...
BOARD_DIR ?= $(FRACTALOS_ROOT)/microkit-sdk-2.1.0/board/$(FRACTALOS_BOARD)/$(SEL4_PROFILE)

# Guest OS selection: buildroot (default), codex, or ubuntu
GUEST_OS ?= buildroot
VMM_DUAL_GUEST ?= 0
FRACTALOS_IMAGES ?= $(FRACTALOS_ROOT)/build/guest-images
...
UBUNTU_KERNEL := $(FRACTALOS_IMAGES)/ubuntu-26.04-aarch64-Image
UBUNTU_INITRD := $(FRACTALOS_IMAGES)/ubuntu-26.04-aarch64-initrd
...
# Official Codex guest: pinned Buildroot base plus a credential-free Codex CLI
# overlay generated by `cargo xtask fetch-guest --os codex`.
CODEX_INITRD := $(FRACTALOS_IMAGES)/fractalos-codex-0.149.1-aarch64-initrd.cpio.gz
CODEX_MANIFEST := $(FRACTALOS_IMAGES)/fractalos-codex-0.149.1-aarch64.json
CODEX_DTS_OVERLAY := $(BUILD_DIR)/fractalos-codex-overlay.dts
...
INITRD_IMAGE := $(CODEX_INITRD)
DTS_OVERLAY_FILE := $(CODEX_DTS_OVERLAY)
else
LINUX_IMAGE  := $(BUILD_DIR)/$(BUILDROOT_LINUX_IMAGE)
...
ifeq ($(VMM_DUAL_GUEST),1)
VMM_CFLAGS += -DFRACTALOS_GUEST_BOTH=1
endif
ifeq ($(GUEST_OS),codex)
VMM_CFLAGS += -DFRACTALOS_GUEST_CODEX=1
endif
...
ifeq ($(GUEST_OS),codex)
$(CODEX_INITRD) $(CODEX_MANIFEST):
	@echo "[VMM] Building pinned official Codex 0.149.1 guest initrd..."
...
$(CODEX_DTS_OVERLAY): $(KERNEL_SRC_DIR)/codex-overlay.dts.in $(KERNEL_SRC_DIR)/vmm.mk $(CODEX_INITRD) $(VMM_CONFIG_STAMP)
	@mkdir -p $(BUILD_DIR)
	@echo "[VMM] Generating official Codex guest overlay..."
	@initrd_size=$$(python3 -c 'import os,sys; print(os.path.getsize(sys.argv[1]))' "$(CODEX_INITRD)"); \
	start=$$(( $(CODEX_INITRD_START) )); \
	limit=$$(( $(CODEX_INITRD_LIMIT) )); \
	end=$$(( start + initrd_size )); \
	if test $$end -ge $$limit; then \
		echo "ERROR: Codex initrd ($$initrd_size bytes) exceeds reserved guest RAM window" >&2; \
		exit 1; \
	fi; \
	end_hex=$$(printf "0x%08x" $$end); \
	sed -e "s|@CODEX_INITRD_END@|$$end_hex|g" $< > $@
...
FREEBSD_DEFAULT_IMAGE := $(FRACTALOS_IMAGES)/freebsd-15.0-aarch64.iso
FREEBSD_RAW_IMAGE ?= $(if $(FRACTALOS_FREEBSD_IMAGE),$(FRACTALOS_FREEBSD_IMAGE),$(if $(FREEBSD_IMAGE),$(FREEBSD_IMAGE),$(FREEBSD_DEFAULT_IMAGE)))
...
FREEBSD_EXTRACT := $(FRACTALOS_ROOT)/tools/extract_freebsd_file.py
...
$(FREEBSD_KERNEL_IMAGE): $(FREEBSD_RAW_IMAGE) $(FREEBSD_EXTRACT)
	...
	@case "$(FREEBSD_RAW_IMAGE)" in \
		*.iso) cargo xtask fetch-guest --os freebsd --output-dir $(FRACTALOS_IMAGES); \
		       cp "$(FRACTALOS_IMAGES)/freebsd-15.0-aarch64-kernel" $@ ;; \
