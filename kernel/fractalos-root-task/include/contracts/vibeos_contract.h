 ...
 * Opcodes: MSG_VIBEOS_* (see fractalos.h)
 *
...
struct vibeos_suspend_req {
    uint32_t handle;
};

struct vibeos_resume_req {
    uint32_t handle;
};
...
struct vibeos_suspend_reply {
    uint32_t ok;
    uint32_t state;             /* VIBEOS_STATE_PAUSED on success */
};

struct vibeos_resume_reply {
    uint32_t ok;
    uint32_t state;             /* VIBEOS_STATE_RUNNING on success */
};
 ...
 * defined in fractalos.h so callers need only one set of constants.
 */
...
/* CH_VIBEOS_ENGINE: authoritative alias for CH_VIBEENGINE (fractalos.h) */
 ...
 * via this channel.  Matches CH_VM_MANAGER in fractalos.h.
 */
