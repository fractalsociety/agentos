 ...
 *
 * Channel: EVENTBUS_CH_* (see fractalos.h)
 * Opcodes: MSG_EVENTBUS_* (see fractalos.h)
 *
 * Invariants:
...
struct eventbus_req_init {
    uint32_t version;           /* caller's fractalos version */
};
...
/* Canonical Agent stream IPC (MSG_EVENTBUS_AGENT_*). Payloads are packed
 * structs; the adapter fills schema/position/hash fields on RECORD. */
#ifndef MSG_EVENTBUS_AGENT_RECORD
#define MSG_EVENTBUS_AGENT_RECORD 0x2702u
#endif
#ifndef MSG_EVENTBUS_AGENT_SEAL
#define MSG_EVENTBUS_AGENT_SEAL   0x2703u
#endif
#ifndef MSG_EVENTBUS_AGENT_REPLAY
#define MSG_EVENTBUS_AGENT_REPLAY 0x2704u
#endif

struct eventbus_req_agent_record {
    uint32_t event_type;
    uint32_t authority_epoch;
    int32_t budget_delta;
    uint32_t flags;
...
struct eventbus_req_agent_seal {
    uint32_t reserved;
};

struct eventbus_req_agent_replay {
    struct eventbus_agent_event_seal seal;
};
...
struct eventbus_reply_agent_record {
    uint32_t status;            /* eventbus_agent_event_error */
    uint64_t position;
    eventbus_event_hash_t event_hash;
};

struct eventbus_reply_agent_seal {
    uint32_t status;
    struct eventbus_agent_event_seal seal;
};

struct eventbus_reply_agent_replay {
    uint32_t status;
    struct eventbus_agent_replay replay;
};
