 ...
 *
 * Channel: CH_AGENT_POOL (see fractalos.h)
 * Opcodes: MSG_AGENTPOOL_* (see fractalos.h)
 *
 * Invariants:
...
struct agentpool_req_alloc_worker {
    uint32_t cap_mask;          /* FRACTALOS_CAP_* bitmask for this worker */
    uint32_t priority;          /* fractalos_priority_t hint */
};
 ...
 * (see fos-681).  Counts satisfy total == busy + idle + faulted.
 *
