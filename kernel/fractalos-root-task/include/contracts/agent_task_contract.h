...
enum agent_task_error {
    ...
    AGENT_TASK_ERR_EVIDENCE_CONSUMED = 19u,
};
...
/* Repair-safe feedback only — never held-out case ids, judge names, scores,
 * timing side-channels, or promotion corpora identifiers. */
enum agent_task_verify_feedback {
    AGENT_TASK_FEEDBACK_NONE = 0u,
    AGENT_TASK_FEEDBACK_TESTS_FAILED = 1u,
    AGENT_TASK_FEEDBACK_EVIDENCE_INCOMPLETE = 2u,
    AGENT_TASK_FEEDBACK_BUDGET = 3u,
    AGENT_TASK_FEEDBACK_POLICY = 4u,
};
...
struct agent_task_reply_verify {
    ...
    uint32_t feedback_code; /* agent_task_verify_feedback — repair-safe only */
    uint32_t reserved;
};

struct agent_task_req_commit {
    struct agent_task_handle task;
    uint32_t authority_epoch;
    uint32_t nonblocking;
    /* Must equal the unconsumed TASK_VERIFY commit_digest. */
    uint8_t candidate_root[AGENT_TASK_DIGEST_BYTES];
    uint32_t evidence_sequence;
    uint32_t reserved;
};

struct agent_task_reply_commit {
    uint32_t status;
    struct agent_task_handle task;
    uint32_t state;
    uint32_t authority_epoch;
    uint32_t evidence_sequence;
    uint32_t consumed; /* 1 when unconsumed successful evidence was spent */
};
...
_Static_assert(sizeof(struct agent_task_reply_verify) == 36u,
               "TASK_VERIFY reply wire size");
_Static_assert(sizeof(struct agent_task_req_commit) == 56u,
               "TASK_COMMIT request wire size");
_Static_assert(sizeof(struct agent_task_reply_commit) == 28u,
               "TASK_COMMIT reply wire size");
