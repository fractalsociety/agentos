 ...
 *
 * The AuthServer PD is the single authentication authority for FractalOS.
 ...
 *   - ADDUSER requires the caller to hold FRACTALOS_CAP_ADMIN.
...
struct auth_server_req_adduser {
    ...
    uint32_t uid;                /* numeric user identifier */
    uint32_t cap_mask;           /* FRACTALOS_CAP_* bitmask for new user */
...
struct auth_server_reply_verify {
    ...
    uint32_t cap_mask;           /* FRACTALOS_CAP_* bitmask for this token */
};
