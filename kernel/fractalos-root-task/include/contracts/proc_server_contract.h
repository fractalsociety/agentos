 ...
 *
 * The ProcServer PD is the process table for FractalOS.  It tracks all
 ...
 *   - OP_PROC_SETCAP requires the caller to hold FRACTALOS_CAP_ADMIN.
...
struct proc_server_req_spawn {
    ...
    uint32_t cap_mask;           /* FRACTALOS_CAP_* bitmask to grant child */
};
...
struct proc_server_req_setcap {
    ...
    uint32_t pid;
    uint32_t cap_mask;           /* new FRACTALOS_CAP_* bitmask */
};
...
struct proc_server_reply_status {
    ...
    uint32_t cap_mask;           /* FRACTALOS_CAP_* bitmask */
};
...
typedef struct __attribute__((packed)) {
    ...
    uint32_t cap_mask;           /* FRACTALOS_CAP_* */
