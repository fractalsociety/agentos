 ...
 * or agentctl) use MSG_CC_* opcodes to query and control the FractalOS stack.
 ...
 *
 * Channel: CH_CC_PD (see fractalos.h)
 * Opcodes: MSG_CC_* (see fractalos.h)
 *
...
/*
 * Drain a guest's serial output as ASCII bytes (fos-vsi).  cc_pd resolves
