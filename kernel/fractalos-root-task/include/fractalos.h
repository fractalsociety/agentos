/*
 * FractalOS - kernel layer common header
 * 
 * All protection domains include this header.
 * It provides:
 * - FractalOS version constants
 * - Channel ID definitions (must match fractalos.system)
 ...
 * In FRACTALOS_TEST_HOST builds the framework.h stub layer provides the types.
...
/* FractalOS version */
#define FRACTALOS_VERSION_MAJOR 0
#define FRACTALOS_VERSION_MINOR 1
#define FRACTALOS_VERSION_PATCH 3
#define FRACTALOS_VERSION_STR   "FractalOS v0.1.3"

/*
 * Channel IDs (must match fractalos.system)
...
#define SWAP_SLOT_BASE_CH     30  /* Channels 30-33 are swap slots (matches fractalos.system) */
...
typedef enum {
    ...
    MSG_FRACTAL_TASK_COMMIT        = 0x2E09, /* fos-gz0.14.8.1: verified-only commit */
    ...
    /* Scoped async actors + persistent mailboxes (0x3000).  Payloads are
     * defined by contracts/actor_mailbox_contract.h (fos-gz0.14.7.1). */
    MSG_ACTOR_SPAWN                = 0x3001,
    MSG_ACTOR_DELEGATE             = 0x3002,
    MSG_ACTOR_MAILBOX_DELIVER      = 0x3003,
    MSG_ACTOR_MAILBOX_POLL         = 0x3004,
    MSG_ACTOR_HANDLE_RESOLVE       = 0x3005,
    MSG_ACTOR_PASSIVATE            = 0x3006, /* fos-gz0.14.7.2 */
    MSG_ACTOR_REACTIVATE           = 0x3007, /* fos-gz0.14.7.2 */
    MSG_ACTOR_MEMORY_STATS         = 0x3008, /* fos-gz0.14.7.2 */

    /* Fractal Agent ISA v0 semantic subprotocol (0x3100).  Kept distinct from
     * the Fractal task-control-plane opcodes at 0x2E00. */
    MSG_AGENT_ISA_SUBMIT           = 0x3101,
    MSG_AGENT_ISA_WAIT             = 0x3102,
    MSG_AGENT_ISA_CANCEL           = 0x3103,
    MSG_AGENT_ISA_STATUS           = 0x3104,
    MSG_AGENT_ISA_TRACE            = 0x3105,
    MSG_AGENT_ISA_DISPATCH_ENQUEUE = 0x3106,
    MSG_AGENT_ISA_DISPATCH_COMPLETE = 0x3107,
    MSG_AGENT_ISA_DISPATCH_CANCEL  = 0x3108,
    MSG_AGENT_ISA_DISPATCH_STATUS  = 0x3109,
    /* fos-gz0.14.1.2: capability-selected semantic adapters (no provider names). */
    MSG_AGENT_ISA_ADAPTER_INSTALL  = 0x310A,
    MSG_AGENT_ISA_ADAPTER_INVOKE   = 0x310B,
    MSG_AGENT_ISA_ADAPTER_STATUS   = 0x310C,

    /* Immutable capability service graph + governed provider swaps (0x3200).
     * Payloads are defined by contracts/service_graph_contract.h
     * (fos-gz0.14.9). Consumers bind by interface hash only. */
    MSG_SERVICE_GRAPH_VALIDATE     = 0x3201,
    MSG_SERVICE_GRAPH_PUBLISH      = 0x3202,
    MSG_SERVICE_GRAPH_BIND         = 0x3203,
    MSG_SERVICE_GRAPH_SWAP         = 0x3204,
    MSG_SERVICE_GRAPH_ROLLBACK     = 0x3205,
    MSG_SERVICE_GRAPH_RESOLVE      = 0x3206,

    /* Immutable shared-space replication + verified merge (0x3300).
     * Payloads are defined by contracts/shared_space_contract.h
     * (fos-gz0.14.10.3). CAS roots only; never CRDT for authority state. */
    MSG_SHARED_SPACE_CREATE        = 0x3301,
    MSG_SHARED_SPACE_PUT_OBJECT    = 0x3302,
    MSG_SHARED_SPACE_GET_OBJECT    = 0x3303,
    MSG_SHARED_SPACE_HAVE_WANT     = 0x3304,
    MSG_SHARED_SPACE_CAS_ROOT      = 0x3305,
    MSG_SHARED_SPACE_BRANCH        = 0x3306,
    MSG_SHARED_SPACE_MERGE         = 0x3307,
    MSG_SHARED_SPACE_STATUS        = 0x3308,

    /* External Fractal Local Gateway fence (0x3400). fos-gz0.14.10.5 —
     * service-cap publish/revoke + pinned daily workspace + task intents.
     * Payloads: contracts/local-gateway/interface.h */
    MSG_LOCAL_GATEWAY_PUBLISH_SERVICE = 0x3401,
    MSG_LOCAL_GATEWAY_REVOKE_SERVICE  = 0x3402,
    MSG_LOCAL_GATEWAY_OPEN_SESSION    = 0x3403,
    MSG_LOCAL_GATEWAY_GET_DAILY       = 0x3404,
    MSG_LOCAL_GATEWAY_SUBMIT_INTENT   = 0x3405,
    MSG_LOCAL_GATEWAY_STATUS          = 0x3406,

    /* Continual harness refinement E1 (0x3500). fos-gz0.14.8 —
     * notes/skills/roles/retry hints: snapshot/evaluate/promote/rollback
     * without WASM. Payloads: contracts/continual_harness_contract.h */
    MSG_CONTINUAL_SNAPSHOT         = 0x3501,
    MSG_CONTINUAL_EVALUATE         = 0x3502,
    MSG_CONTINUAL_PROMOTE          = 0x3503,
    MSG_CONTINUAL_ROLLBACK         = 0x3504,
    MSG_CONTINUAL_STATUS           = 0x3505,
    MSG_CONTINUAL_QUERY_HELD_OUT   = 0x3506, /* always denied — leakage fence */
...
/*
 * Fine-grained FRACTALOS_CAP_* capability constants.
 ...
 * They appear as cap_class fields in fractalos.system annotations and are
 * the values agents declare in their fractalos.capabilities custom section.
 */
#define FRACTALOS_CAP_COMPUTE      0x01u  /* CPU time budget */
#define FRACTALOS_CAP_MEMORY       0x02u  /* heap allocation */
#define FRACTALOS_CAP_OBJECTSTORE  0x04u  /* AgentFS access */
#define FRACTALOS_CAP_NETWORK      0x08u  /* network endpoints */
#define FRACTALOS_CAP_SPAWN        0x10u  /* spawn child agents */
#define FRACTALOS_CAP_AUDIT        0x20u  /* read audit log */
#define FRACTALOS_CAP_SWAP_WRITE   0x40u  /* write WASM to swap region (controller only) */
#define FRACTALOS_CAP_SWAP_READ    0x80u  /* read/execute WASM from swap region (swap slots) */
...
#  define FRACTALOS_LOG(fmt, ...) \
     do { \
         sel4_dbg_puts("[fractalos] "); \
         sel4_dbg_puts(fmt);          \
         sel4_dbg_puts("\n");         \
     } while(0)
#else
#  define FRACTALOS_LOG(fmt, ...) do {} while(0)
#endif

/* Always-on critical logs */
#define FRACTALOS_CRIT(fmt) \
    do { \
        sel4_dbg_puts("[CRIT] "); \
        sel4_dbg_puts(fmt);       \
        sel4_dbg_puts("\n");      \
...
typedef struct __attribute__((packed)) {
    uint32_t magic;          /* 0xA6E7_0B05 = "FRACTALOS" */
    uint32_t version;
...
} fractalos_ring_header_t;

#define FRACTALOS_RING_MAGIC 0xA6E70B05
...
void fractalos_log_boot(const char *pd_name);
void fractalos_log_info(const char *pd, const char *msg);
void fractalos_log_channel(const char *pd, uint32_t ch);
void fractalos_log_fault(const char *pd, fractalos_fault_t *f);
...
static inline void log_drain_write(uint32_t slot, uint32_t pd_id, const char *msg)
{
     ...
     * In test builds (FRACTALOS_TEST_HOST) we skip the IPC entirely since
...
#define MSG_EVENTBUS_AGENT_RECORD       0x2702  /* canonical stream append */
#define MSG_EVENTBUS_AGENT_SEAL         0x2703  /* seal head + count */
#define MSG_EVENTBUS_AGENT_REPLAY       0x2704  /* verify sealed range */
...
#define MSG_AGENT_TASK_COMMIT             MSG_FRACTAL_TASK_COMMIT
...
#define MSG_AGENTFS_DESC_PERSIST        0x1008  /* persist event descriptor by payload_root */
#define MSG_AGENTFS_DESC_RESOLVE        0x1009  /* resolve descriptor bytes by payload_root */
 ...
 *
 * fractalos_abi.h is generated by tools/gen-abi from tools/abi_spec.toml. It is
 * the collision-checked source of truth for per-PD IPC opcodes and channel
 * IDs (gen-abi fails the build on any duplicate opcode/channel or missing
 * contract coverage).
 *
 * The hand-written MSG_, OP_ and CH_ constants above remain the names
 * consumed by PD source. FRACTALOS_ABI_CHECK_LEGACY makes the generated header
 * emit a _Static_assert per entry binding each generated ABI value to the
 * legacy constant it mirrors, so a stale constant above fails the C build.
 *
 * Regenerate: cargo run -p gen-abi -- tools/abi_spec.toml \
 *               -o kernel/fractalos-root-task/include/fractalos_abi.h
 * (or `cargo xtask gen-abi`).
 * ══════════════════════════════════════════════════════════════════════════ */
#define FRACTALOS_ABI_CHECK_LEGACY
#include "fractalos_abi.h"
