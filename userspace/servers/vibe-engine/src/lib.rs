//! Vibe Engine — the service hot-swap protocol for FractalOS
//!
//! This is the core innovation that makes FractalOS more than just "seL4 for agents."
...
//! and careful review. FractalOS services are designed to be hot-swappable by agents
...
pub fn validate_proposal(proposal: &SwapProposal, contract: &ServiceContract) -> ValidationResult {
    ...
    if proposal.format == BinaryFormat::Wasm {
        let wasm_valid =
            proposal.binary.len() >= 8 && proposal.binary[0..4] == [0x00, 0x61, 0x73, 0x6D]; // WASM magic
        checks.push(ValidationCheck {
            name: "wasm_module_valid".into(),
            passed: wasm_valid,
            detail: if wasm_valid {
                "Valid WASM magic number".into()
            } else {
                "Invalid WASM module header".into()
            },
        });

        // 5b. __fractalos_caps custom section policy enforcement
        //
        // Parses the WASM binary for a custom section named "__fractalos_caps"
        ...
        //   name_len: u32 (LEB128)
        //   name: "__fractalos_caps" (UTF-8)
        //   payload: JSON CapabilityManifest
        if wasm_valid {
            match parse_fractalos_caps_section(&proposal.binary) {
                Ok(Some(manifest)) => {
                ...
                Ok(None) => {
                    // No __fractalos_caps section — allow but warn (legacy/unsigned modules)
                    checks.push(ValidationCheck {
                        name: "capability_manifest".into(),
                        passed: true,
                        detail:
                            "No __fractalos_caps section — capabilities unconstrained (legacy mode)"
                                .into(),
                    });
                }
                Err(e) => {
                    checks.push(ValidationCheck {
                        name: "capability_manifest".into(),
                        passed: false,
                        detail: alloc::format!("Malformed __fractalos_caps section: {}", e),
                    });
                }
            }
        }
...
/// Capability manifest declared in a WASM module's `__fractalos_caps` custom section.
...
/// Parse the `__fractalos_caps` custom section from a WASM binary.
///
...
pub fn parse_fractalos_caps_section(wasm: &[u8]) -> Result<Option<CapabilityManifest>, String> {
    const SECTION_NAME: &[u8] = b"__fractalos_caps";
    ...
    while pos < wasm.len() {
        ...
        let (section_size, bytes_read) =
            read_leb128_u32(wasm, pos).ok_or("Truncated section size")?;
        pos += bytes_read;
        ...
        if section_id == 0x00 {
            ...
            let (name_len, name_bytes) =
                read_leb128_u32(wasm, pos).ok_or("Truncated custom section name length")?;
    ...
    Ok(None) // No __fractalos_caps section found
}
...
/// Parse the JSON payload of an `__fractalos_caps` section.
...
fn parse_caps_json(payload: &[u8]) -> Result<CapabilityManifest, String> {
    ...
    let s = core::str::from_utf8(payload).map_err(|_| "Capability manifest is not valid UTF-8")?;
...
pub fn check_capability_policy(
    ...
    let allowed_caps: Vec<&str> = contract
        .required_caps
        .iter()
...
///
/// With the `std` feature, this drives a real `fractalos_sim::SimEngine`:
...
fn run_sandbox_std(target_service: &str, wasm_bytes: &[u8]) -> SandboxResult {
    let engine = fractalos_sim::SimEngine::new();
...
fn start_rollback_monitor(proposal_id: u64, wasm_bytes: Vec<u8>, rollback_window_secs: u64) {
    #[cfg(feature = "std")]
    {
        std::thread::spawn(move || {
            let deadline =
                std::time::Instant::now() + std::time::Duration::from_secs(rollback_window_secs);
...
/// Register the default service contracts for FractalOS
pub fn register_default_contracts(engine: &mut VibeEngine) {
    engine.register_contract(ServiceContract {
        ...
        required_caps: vec![
            CapRequirement {
                cap_type: "memory".into(),
                reason: "Content storage pool".into(),
                optional: false,
            },
            CapRequirement {
                cap_type: "log".into(),
                reason: "Audit logging".into(),
                optional: true,
            },
        ],
    ...
    engine.register_contract(ServiceContract {
        ...
        required_caps: vec![
            CapRequirement {
                cap_type: "memory".into(),
                reason: "Ring buffer".into(),
                optional: false,
            },
            CapRequirement {
                cap_type: "notification".into(),
                reason: "Async delivery".into(),
                optional: false,
            },
        ],
        performance: PerformanceBaseline {
    ...
    engine.register_contract(ServiceContract {
        ...
        required_caps: vec![CapRequirement {
            cap_type: "memory".into(),
            reason: "Tool registry".into(),
            optional: false,
        }],
        performance: PerformanceBaseline {
            max_latency_us: 500,
    ...
    engine.register_contract(ServiceContract {
        ...
        required_caps: vec![
            CapRequirement {
                cap_type: "network".into(),
                reason: "HTTP API access".into(),
                optional: false,
            },
            CapRequirement {
                cap_type: "memory".into(),
                reason: "Prompt cache".into(),
                optional: false,
            },
        ],
        performance: PerformanceBaseline {
...
mod tests {
    ...
    fn test_full_swap_lifecycle() {
        ...
        assert!(valid);
        assert_eq!(
            *engine.proposal_status(id).unwrap(),
    ...
    fn test_parse_caps_section_absent() {
        ...
        let result = parse_fractalos_caps_section(&wasm).unwrap();
        assert!(result.is_none(), "No caps section should return None");
    }

    #[test]
    fn test_parse_caps_section_present() {
        // Build a WASM binary with an __fractalos_caps custom section
        let section_name = b"__fractalos_caps";
        ...
        let result = parse_fractalos_caps_section(&wasm).unwrap();
        assert!(result.is_some(), "Should find __fractalos_caps section");
    ...
    fn test_capability_policy_denied_cap() {
        ...
        assert!(
            !result.allowed,
            "Should be denied: network not in storage policy"
        );
