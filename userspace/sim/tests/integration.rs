//! Integration tests for fractalos-sim.
//!
...
const HELLO_AGENT_WASM: &[u8] = &[
    ...
    // ── Section 1: Type ────────────────────────────────────────────────────
    // 6 function types
    0x01, 0x23, 0x06, 0x60, 0x00, 0x00, // type 0: () -> ()
    ...
    // ── Section 2: Import ──────────────────────────────────────────────────
    // 3 host functions
    0x02, 0x3d, 0x03, // env.aos_log : type 1
    0x03, 0x65, 0x6e, 0x76, 0x07, 0x61, 0x6f, 0x73, 0x5f, 0x6c, 0x6f, 0x67, 0x00, 0x01,
    // env.aos_event_publish : type 2
    0x03, 0x65, 0x6e, 0x76, 0x11, 0x61, 0x6f, 0x73, 0x5f, 0x65, 0x76, 0x65, 0x6e, 0x74, 0x5f, 0x70,
    0x75, 0x62, 0x6c, 0x69, 0x73, 0x68, 0x00, 0x02, // env.microkit_ppcall : type 3
    0x03, 0x65, 0x6e, 0x76, 0x0f, 0x6d, 0x69, 0x63, 0x72, 0x6f, 0x6b, 0x69, 0x74, 0x5f, 0x70, 0x70,
    0x63, 0x61, 0x6c, 0x6c, 0x00, 0x03,
    // ── Section 3: Function ────────────────────────────────────────────────
    ...
    // ── Section 7: Export ──────────────────────────────────────────────────
    0x07, 0x2d, 0x04, 0x06, 0x6d, 0x65, 0x6d, 0x6f, 0x72, 0x79, 0x02,
    0x00, // "memory"       mem[0]
    0x04, 0x69, 0x6e, 0x69, 0x74, 0x00, 0x03, // "init"         func[3]
    0x0a, 0x68, 0x61, 0x6e, 0x64, 0x6c, 0x65, 0x5f, 0x70, 0x70, 0x63, 0x00,
    0x04, // "handle_ppc"   func[4]
    0x0c, 0x68, 0x65, 0x61, 0x6c, 0x74, 0x68, 0x5f, 0x63, 0x68, 0x65, 0x63, 0x6b, 0x00,
    0x05, // "health_check" func[5]
    // ── Section 10: Code ───────────────────────────────────────────────────
    0x0a, 0x25, 0x03, // body func[3] = init()
    0x12, 0x00, 0x41, 0x00, 0x41, 0x04, 0x10, 0x00, // aos_log(0, 4)
    0x41, 0x10, 0x41, 0x05, 0x41, 0x20, 0x41, 0x05, 0x10,
    0x01, // aos_event_publish(16,5,32,5)
    0x0b, // body func[4] = handle_ppc(i64×5)
    0x0b, 0x00, 0x41, 0x00, // i32.const 0   (channel)
    0x20, 0x00, // local.get 0   (mr0)
    0x41, 0x01, // i32.const 1   (mr_count)
    0x10, 0x02, // call microkit_ppcall
    0x1a, // drop i64 result
    0x0b, // body func[5] = health_check() -> i32
    0x04, 0x00, 0x41, 0x00, 0x0b,
    // ── Section 11: Data ───────────────────────────────────────────────────
    0x0b, 0x1e, 0x03, 0x00, 0x41, 0x00, 0x0b, 0x04, 0x69, 0x6e, 0x69,
    0x74, // offset 0:  "init"
...
fn test_init_publishes_event() {
    ...
    assert_eq!(
        ...
        "event payload should be b\"world\", got {:?}",
        first.payload
    );
    assert_eq!(
...
fn test_handle_ppc_captured() {
    ...
    runner
        .handle_ppc(0xA0, 1, 2, 3, 4)
        .expect("handle_ppc() should not trap");
...
fn test_handle_ppc_multiple_calls_accumulate() {
    ...
    assert_eq!(
        call_log.len(),
        3,
        "three ppcalls should accumulate in call_log"
    );
...
fn test_health_check_returns_zero() {
    ...
    let result = runner
        .health_check()
        .expect("health_check() should not trap");
...
fn test_full_lifecycle() {
    ...
    assert!(
        log.iter().any(|l| l.contains("init")),
        "log_lines should contain 'init', got: {:?}",
        log
    );
    ...
    assert!(
        notify_log.is_empty(),
        "no notifications should have been sent, got: {:?}",
        notify_log
    );
...
fn test_two_agents_share_eventbus() {
    ...
    let sources: Vec<_> = hello_events
        .iter()
        .filter_map(|e| e.source.as_deref())
        .collect();
    assert!(
        sources.contains(&"agent-a"),
        "agent-a should appear as source"
    );
    assert!(
        sources.contains(&"agent-b"),
        "agent-b should appear as source"
    );
}
